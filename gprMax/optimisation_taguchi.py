...
def run_opt_sim(args, inputfile, usernamespace):
    ...
    print(
        "Orthogonal array: {:g} experiments per iteration, {:g} parameters ({:g} will be used), {:g} levels, and strength {:g}".format(
            N, cols, k, s, t
        )
    )
    ...
    print(
        'Fitness function "{}" with stopping criterion {:g}'.format(
    ...
    while iteration < maxiterations:
        ...
        os.rename(
            outputfile,
            os.path.splitext(outputfile)[0] + "_final" + str(iteration + 1) + ".out",
        )
        ...
        if iteration > 2:
            fitnessvaluesclose = (
                np.abs(
                    fitnessvalueshist[iteration - 2] - fitnessvalueshist[iteration - 1]
                )
    ...
    print(
        "History of optimal parameter values {} and of fitness values {}\n".format(
            dict(optparamshist), fitnessvalueshist
        )
    )
    ...
    while x < len(inputlines):
        if inputlines[x].startswith("#taguchi:"):
            ...
            while not inputlines[x].startswith("#end_taguchi:"):
                ...
                if x == len(inputlines):
                    raise CmdInputError(
                        "Cannot find the end of the Taguchi code block, i.e. missing #end_taguchi: command."
                    )
...
def construct_OA(optparams):
    ...
    oadirectory = os.path.join(
        os.path.dirname(os.path.abspath(__file__)),
        os.pardir,
        "user_libs",
        "optimisation_taguchi",
    )
    ...
    if k <= 4:
        ...
        print(
            Fore.RED
            + "WARNING: Optimising more than 7 parameters is currently an experimental feature!"
            + Style.RESET_ALL
        )
...
def calculate_ranges_experiments(
    optparams, optparamsinit, levels, levelsopt, levelsdiff, OA, N, k, s, i
):
    ...
    for p in range(k):
        ...
        if i == 0:
            levels[1, p] = (
                (optparamsinit[p][1][1] - optparamsinit[p][1][0]) / 2
            ) + optparamsinit[p][1][0]
...
def plot_optimisation_history(fitnessvalueshist, optparamshist, optparamsinit):
    ...
    fig, ax = plt.subplots(
        subplot_kw=dict(xlabel="Iterations", ylabel="Fitness value"),
        num="History of fitness values",
        figsize=(20, 10),
        facecolor="w",
        edgecolor="w",
    )
    ...
    for key, value in optparamshist.items():
        fig, ax = plt.subplots(
            subplot_kw=dict(xlabel="Iterations", ylabel="Parameter value"),
            num="History of " + key + " parameter",
            figsize=(20, 10),
            facecolor="w",
            edgecolor="w",
        )
