...
def process_multicmds(multicmds, G):
        ...
        for cmdinstance in multicmds[cmdname]:
            ...
            if tmp[0].lower() not in Waveform.types:
                raise CmdInputError(
                    ...
                    + " ".join(tmp)
                    + "'"
                    + " must have one of the following types {}".format(
                        ",".join(Waveform.types)
                    )
                )
            if float(tmp[2]) <= 0:
            ...
            if any(x.ID == tmp[3] for x in G.waveforms):
                raise CmdInputError(
                    ...
                    + " ".join(tmp)
                    + "'"
                    + " with ID {} already exists".format(tmp[3])
                )
            ...
            if G.messages:
                print(
                    "Waveform {} of type {} with maximum amplitude scaling {:g}, frequency {:g}Hz created.".format(
                        w.ID, w.type, w.amp, w.freq
                    )
                )
        ...
        for cmdinstance in multicmds[cmdname]:
            ...
            if "2D TMx" in G.mode and (polarisation == "y" or polarisation == "z"):
                raise CmdInputError(
                    ...
                    + " ".join(tmp)
                    + "'"
                    + " polarisation must be x in 2D TMx mode"
                )
                ...
                raise CmdInputError(
                    ...
                    + " ".join(tmp)
                    + "'"
                    + " polarisation must be y in 2D TMy mode"
                )
                ...
                raise CmdInputError(
                    ...
                    + " ".join(tmp)
                    + "'"
                    + " polarisation must be z in 2D TMz mode"
                )
            ...
            if (
                ...
                print(
                    ...
                    + " ".join(tmp)
                    + "'"
                    + " sources and receivers should not normally be positioned within the PML."
                    + Style.RESET_ALL
                )
            if resistance < 0:
                raise CmdInputError(
            ...
            if not any(x.ID == tmp[5] for x in G.waveforms):
                raise CmdInputError(
                    ...
                    + " ".join(tmp)
                    + "'"
                    + " there is no waveform with the identifier {}".format(tmp[5])
                )
            ...
            if G.messages:
                print(
                    "Voltage source with polarity {} at {:g}m, {:g}m, {:g}m, resistance {:.1f} Ohms,".format(
                        v.polarisation,
                        v.xcoord * G.dx,
                        v.ycoord * G.dy,
                        v.zcoord * G.dz,
                    ...
                    + startstop
                    + "using waveform {} created.".format(v.waveformID)
                )
        ...
        for cmdinstance in multicmds[cmdname]:
            ...
            if not any(x.ID == tmp[4] for x in G.waveforms):
                raise CmdInputError(
                    ...
                    + " ".join(tmp)
                    + "'"
                    + " there is no waveform with the identifier {}".format(tmp[4])
                )
            ...
            if G.messages:
                if G.mode == "2D":
                    print(
                        "Hertzian dipole is a line source in 2D with polarity {} at {:g}m, {:g}m, {:g}m,".format(
                            h.polarisation,
                            h.xcoord * G.dx,
                            h.ycoord * G.dy,
                            h.zcoord * G.dz,
                        ...
                        + startstop
                        + "using waveform {} created.".format(h.waveformID)
                    )
                else:
                    print(
                        "Hertzian dipole with polarity {} at {:g}m, {:g}m, {:g}m,".format(
                            h.polarisation,
                            h.xcoord * G.dx,
                            h.ycoord * G.dy,
                            h.zcoord * G.dz,
    ...
    if multicmds[cmdname] is not None:
        for cmdinstance in multicmds[cmdname]:
            ...
            if G.messages:
                print(
                    "Magnetic dipole with polarity {} at {:g}m, {:g}m, {:g}m,".format(
                        m.polarisation,
                        m.xcoord * G.dx,
                        m.ycoord * G.dy,
                        m.zcoord * G.dz,
                    ...
                    + startstop
                    + "using waveform {} created.".format(m.waveformID)
                )
        ...
        for cmdinstance in multicmds[cmdname]:
            ...
            if G.gpu is not None:
                raise CmdInputError(
                    ...
                    + " ".join(tmp)
                    + "'"
                    + " A #transmission_line cannot currently be used with GPU solving. Consider using a #voltage_source instead."
                )
            ...
            if resistance <= 0 or resistance >= z0:
                raise CmdInputError(
                    ...
                    + " ".join(tmp)
                    + "'"
                    + " requires a resistance greater than zero and less than the impedance of free space, i.e. 376.73 Ohms"
                )
            ...
            if G.messages:
                print(
                    "Transmission line with polarity {} at {:g}m, {:g}m, {:g}m, resistance {:.1f} Ohms,".format(
                        t.polarisation,
                        t.xcoord * G.dx,
                        t.ycoord * G.dy,
                        t.zcoord * G.dz,
                    ...
                    + startstop
                    + "using waveform {} created.".format(t.waveformID)
                )
    ...
    if multicmds[cmdname] is not None:
        for cmdinstance in multicmds[cmdname]:
                ...
                for field in tmp[4::]:
                    if field in allowableoutputs:
                        ...
                        raise CmdInputError(
                            ...
                            + " ".join(tmp)
                            + "'"
                            + " contains an output type that is not allowable. Allowable outputs in current context are {}".format(
                                allowableoutputs
                            )
                        )
            ...
            if G.messages:
                print(
                    "Receiver at {:g}m, {:g}m, {:g}m with output component(s) {} created.".format(
                        r.xcoord * G.dx,
                        r.ycoord * G.dy,
                        r.zcoord * G.dz,
                        ", ".join(r.outputs),
        ...
        for cmdinstance in multicmds[cmdname]:
            ...
            if dx < 0 or dy < 0 or dz < 0:
                raise CmdInputError(
                    ...
                    + " ".join(tmp)
                    + "'"
                    + " the step size should not be less than zero"
                )
            if dx < 1:
                if dx == 0:
                    dx = 1
                    ...
                    raise CmdInputError(
                        ...
                        + " ".join(tmp)
                        + "'"
                        + " the step size should not be less than the spatial discretisation"
                    )
            if dy < 1:
                if dy == 0:
                    dy = 1
            ...
            if G.messages:
                print(
                    "Receiver array {:g}m, {:g}m, {:g}m, to {:g}m, {:g}m, {:g}m with steps {:g}m, {:g}m, {:g}m".format(
                        xs * G.dx,
                        ys * G.dy,
                        zs * G.dz,
                        xf * G.dx,
    ...
    if multicmds[cmdname] is not None:
        for cmdinstance in multicmds[cmdname]:
            ...
            # If real floating point value given
                ...
                if time > 0:
                    ...
                    raise CmdInputError(
                        ...
                        + " ".join(tmp)
                        + "'"
                        + " time value must be greater than zero"
                    )
            ...
            if G.messages:
                print(
                    "Snapshot from {:g}m, {:g}m, {:g}m, to {:g}m, {:g}m, {:g}m, discretisation {:g}m, {:g}m, {:g}m, at {:g} secs with filename {} created.".format(
                        xs * G.dx,
                        ys * G.dy,
                        zs * G.dz,
                        xf * G.dx,
        ...
        for cmdinstance in multicmds[cmdname]:
            ...
            if float(tmp[0]) < 1:
                raise CmdInputError(
                    ...
                    + " ".join(tmp)
                    + "'"
                    + " requires a positive value of one or greater for static (DC) permittivity"
                )
            if tmp[1] != "inf":
                se = float(tmp[1])
                if se < 0:
                    raise CmdInputError(
                        ...
                        + " ".join(tmp)
                        + "'"
                        + " requires a positive value for conductivity"
                    )
            else:
                se = float("inf")
            if float(tmp[2]) < 1:
                raise CmdInputError(
                    ...
                    + " ".join(tmp)
                    + "'"
                    + " requires a positive value of one or greater for permeability"
                )
            if float(tmp[3]) < 0:
                raise CmdInputError(
                    "'"
                    ...
                    + " ".join(tmp)
                    + "'"
                    + " requires a positive value for magnetic conductivity"
                )
            if any(x.ID == tmp[4] for x in G.materials):
                raise CmdInputError(
                    ...
                    + " ".join(tmp)
                    + "'"
                    + " with ID {} already exists".format(tmp[4])
                )
            ...
            if G.messages:
                tqdm.write(
                    "Material {} with eps_r={:g}, sigma={:g} S/m; mu_r={:g}, sigma*={:g} Ohm/m created.".format(
                        m.ID, m.er, m.se, m.mr, m.sm
                    )
                )
        ...
        for cmdinstance in multicmds[cmdname]:
            ...
            if int(tmp[0]) < 0:
                raise CmdInputError(
                    ...
                    + " ".join(tmp)
                    + "'"
                    + " requires a positive value for number of poles"
                )
            poles = int(tmp[0])
            ...
            for material in materials:
                ...
                for pole in range(1, 2 * poles, 2):
                    ...
                    if float(tmp[pole]) > 0:
                        ...
                        raise CmdInputError(
                            ...
                            + " ".join(tmp)
                            + "'"
                            + " requires positive values for the permittivity difference."
                        )
                ...
                if G.messages:
                    tqdm.write(
                        "Debye disperion added to {} with delta_eps_r={}, and tau={} secs created.".format(
                            material.ID,
                            ", ".join(
                                "%4.2f" % deltaer for deltaer in material.deltaer
                            ),
                            ", ".join("%4.3e" % tau for tau in material.tau),
                        )
                    )
        ...
        for cmdinstance in multicmds[cmdname]:
            ...
            for material in materials:
                ...
                for pole in range(1, 3 * poles, 3):
                    if (
                        float(tmp[pole]) > 0
                        and float(tmp[pole + 1]) > G.dt
                        and float(tmp[pole + 2]) > G.dt
                    ):
                        material.deltaer.append(float(tmp[pole]))
                        material.tau.append(float(tmp[pole + 1]))
                        ...
                        raise CmdInputError(
                            ...
                            + " ".join(tmp)
                            + "'"
                            + " requires positive values for the permittivity difference and frequencies, and associated times that are greater than the time step for the model."
                        )
                if material.poles > Material.maxpoles:
                    Material.maxpoles = material.poles
                ...
                if G.messages:
                    tqdm.write(
                        "Lorentz disperion added to {} with delta_eps_r={}, omega={} secs, and gamma={} created.".format(
                            material.ID,
                            ", ".join(
                                "%4.2f" % deltaer for deltaer in material.deltaer
                            ),
                            ", ".join("%4.3e" % tau for tau in material.tau),
                            ", ".join("%4.3e" % alpha for alpha in material.alpha),
                        )
                    )
    ...
    if multicmds[cmdname] is not None:
        for cmdinstance in multicmds[cmdname]:
            ...
            for material in materials:
                ...
                for pole in range(1, 2 * poles, 2):
                    if float(tmp[pole]) > 0 and float(tmp[pole + 1]) > G.dt:
                        ...
                        raise CmdInputError(
                            ...
                            + " ".join(tmp)
                            + "'"
                            + " requires positive values for the frequencies, and associated times that are greater than the time step for the model."
                        )
                if material.poles > Material.maxpoles:
                    Material.maxpoles = material.poles
                ...
                if G.messages:
                    tqdm.write(
                        "Drude disperion added to {} with omega={} secs, and gamma={} secs created.".format(
                            material.ID,
                            ", ".join("%4.3e" % tau for tau in material.tau),
                            ", ".join("%4.3e" % alpha for alpha in material.alpha),
                        )
        ...
        for cmdinstance in multicmds[cmdname]:
            ...
            if float(tmp[0]) < 0:
                raise CmdInputError(
                    ...
                    + " ".join(tmp)
                    + "'"
                    + " requires a positive value for the sand fraction"
                )
            if float(tmp[1]) < 0:
                raise CmdInputError(
                    "'"
                    ...
                    + " ".join(tmp)
                    + "'"
                    + " requires a positive value for the clay fraction"
                )
            if float(tmp[2]) < 0:
                raise CmdInputError(
                    "'"
                    ...
                    + " ".join(tmp)
                    + "'"
                    + " requires a positive value for the bulk density"
                )
            if float(tmp[3]) < 0:
                raise CmdInputError(
                    "'"
                    ...
                    + " ".join(tmp)
                    + "'"
                    + " requires a positive value for the sand particle density"
                )
            if float(tmp[4]) < 0:
                raise CmdInputError(
                    "'"
                    ...
                    + " ".join(tmp)
                    + "'"
                    + " requires a positive value for the lower limit of the water volumetric fraction"
                )
            if float(tmp[5]) < 0:
                raise CmdInputError(
                    "'"
                    ...
                    + " ".join(tmp)
                    + "'"
                    + " requires a positive value for the upper limit of the water volumetric fraction"
                )
            if any(x.ID == tmp[6] for x in G.mixingmodels):
                raise CmdInputError(
                    ...
                    + " ".join(tmp)
                    + "'"
                    + " with ID {} already exists".format(tmp[6])
                )
            ...
            if G.messages:
                print(
                    "Mixing model (Peplinski) used to create {} with sand fraction {:g}, clay fraction {:g}, bulk density {:g}g/cm3, sand particle density {:g}g/cm3, and water volumetric fraction {:g} to {:g} created.".format(
                        s.ID, s.S, s.C, s.rb, s.rs, s.mu[0], s.mu[1]
                    )
                )
        ...
        for cmdinstance in multicmds[cmdname]:
            ...
            if dx > G.nx or dy > G.ny or dz > G.nz:
                raise CmdInputError(
                    ...
                    + " ".join(tmp)
                    + "'"
                    + " the step size should be less than the domain size"
                )
            if dx < 1 or dy < 1 or dz < 1:
                raise CmdInputError(
                    "'"
            ...
            if tmp[10].lower() != "n" and tmp[10].lower() != "f":
                raise CmdInputError(
                    ...
                    + " ".join(tmp)
                    + "'"
                    + " requires type to be either n (normal) or f (fine)"
                )
            if tmp[10].lower() == "f" and (
                dx * G.dx != G.dx or dy * G.dy != G.dy or dz * G.dz != G.dz
            ):
                raise CmdInputError(
                    "'"
                    + cmdname
                    ...
                    + " ".join(tmp)
                    + "'"
                    + " requires the spatial discretisation for the geometry view to be the same as the model for geometry view of type f (fine)"
                )
            ...
            if G.messages:
                print(
                    "Geometry view from {:g}m, {:g}m, {:g}m, to {:g}m, {:g}m, {:g}m, discretisation {:g}m, {:g}m, {:g}m, with filename base {} created.".format(
                        xs * G.dx,
                        ys * G.dy,
                        zs * G.dz,
                        xf * G.dx,
    ...
    if multicmds[cmdname] is not None:
        for cmdinstance in multicmds[cmdname]:
            ...
            if G.messages:
                print(
                    "Geometry objects in the volume from {:g}m, {:g}m, {:g}m, to {:g}m, {:g}m, {:g}m, will be written to {}, with materials written to {}".format(
                        xs * G.dx,
                        ys * G.dy,
                        zs * G.dz,
                        xf * G.dx,
    ...
    if multicmds[cmdname] is not None:
        if len(multicmds[cmdname]) > 2:
            raise CmdInputError(
                ...
                + " ".join(tmp)
                + "'"
                + " can only be used up to two times, for up to a 2nd order PML"
            )
        for cmdinstance in multicmds[cmdname]:
            tmp = cmdinstance.split()
            if len(tmp) != 12:
            ...
            if (
                tmp[0] not in CFSParameter.scalingprofiles.keys()
                or tmp[4] not in CFSParameter.scalingprofiles.keys()
                or tmp[8] not in CFSParameter.scalingprofiles.keys()
            ):
                raise CmdInputError(
                    "'"
                    + cmdname
                    ...
                    + " ".join(tmp)
                    + "'"
                    + " must have scaling type {}".format(
                        ",".join(CFSParameter.scalingprofiles.keys())
                    )
                )
            if (
                tmp[1] not in CFSParameter.scalingdirections
                or tmp[5] not in CFSParameter.scalingdirections
                or tmp[9] not in CFSParameter.scalingdirections
            ):
                raise CmdInputError(
                    "'"
                    + cmdname
                    ...
                    + "'"
                    + " must have scaling type {}".format(
                        ",".join(CFSParameter.scalingdirections)
                    )
                )
            if (
                float(tmp[2]) < 0
                ...
                raise CmdInputError(
                    ...
                    + " ".join(tmp)
                    + "'"
                    + " minimum and maximum scaling values must be positive"
                )
            if float(tmp[6]) < 1 and G.pmlformulation == "HORIPML":
                raise CmdInputError(
                    ...
                    + " ".join(tmp)
                    + "'"
                    + " minimum scaling value for kappa must be greater than or equal to one"
                )
            ...
            if G.messages:
                print(
                    "PML CFS parameters: alpha (scaling: {}, scaling direction: {}, min: {:g}, max: {:g}), kappa (scaling: {}, scaling direction: {}, min: {:g}, max: {:g}), sigma (scaling: {}, scaling direction: {}, min: {:g}, max: {}) created.".format(
                        cfsalpha.scalingprofile,
                        cfsalpha.scalingdirection,
                        cfsalpha.min,
                        cfsalpha.max,
                        cfskappa.scalingprofile,
                        cfskappa.scalingdirection,
                        cfskappa.min,
                        cfskappa.max,
                        cfssigma.scalingprofile,
                        cfssigma.scalingdirection,
                        cfssigma.min,
                        cfssigma.max,
                    )
                )
