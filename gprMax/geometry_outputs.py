...
class GeometryView(object):
    ...
    def __init__(
        ...
        if self.fileext == ".vti":
            ...
            self.datawritesize = (
                np.dtype(np.uint32).itemsize * self.vtk_ncells
            ...
            self.vtk_connectivity_offset = round_value(
                (
                    self.vtk_numpoints
            ...
            self.vtk_offsets_offset = round_value(
                self.vtk_connectivity_offset
                + (
                    self.vtk_numlines
            ...
            self.vtk_materials_offset = round_value(
                self.vtk_offsets_offset
    ...
    def set_filename(self, appendmodelnumber, G):
        ...
        self.filename = os.path.abspath(
            os.path.join(G.inputdirectory, self.basefilename + appendmodelnumber)
        )
            ...
            with open(self.filename, "wb") as f:
                ...
                f.write(
                    '<PolyData>\n<Piece NumberOfPoints="{}" NumberOfVerts="0" NumberOfLines="{}" NumberOfStrips="0" NumberOfPolys="0">\n'.format(
                        self.vtk_numpoints, self.vtk_numlines
                    ).encode("utf-8")
                )
...
class GeometryObjects(object):
    ...
    def write_hdf5(self, G, pbar):
        ...
        fdata = h5py.File(
            os.path.abspath(os.path.join(G.inputdirectory, self.filename)), "w"
        )
        ...
        minmat = np.amin(
            G.ID[:, self.xs : self.xf, self.ys : self.yf, self.zs : self.zf]
        )
        maxmat = np.amax(
            G.ID[:, self.xs : self.xf, self.ys : self.yf, self.zs : self.zf]
        )
        fdata["/data"] = (
            G.solid[self.xs : self.xf, self.ys : self.yf, self.zs : self.zf].astype(
                "int16"
            )
            - minmat
        )
        ...
        fdata["/rigidE"] = G.rigidE[
            :, self.xs : self.xf, self.ys : self.yf, self.zs : self.zf
        ]
        ...
        fdata["/ID"] = (
            G.ID[:, self.xs : self.xf, self.ys : self.yf, self.zs : self.zf] - minmat
        )
        ...
        fmaterials = open(
            os.path.abspath(os.path.join(G.inputdirectory, self.materialsfilename)), "w"
        )
                ...
                if material.numID == numID:
                    ...
                    if material.poles > 0:
                        if "debye" in material.type:
                            dispersionstr = "#add_dispersion_debye: {:g} ".format(
                                material.poles
                            )
                            for pole in range(material.poles):
                                dispersionstr += "{:g} {:g} ".format(
                                    material.deltaer[pole], material.tau[pole]
                                )
                        elif "lorenz" in material.type:
                            dispersionstr = "#add_dispersion_lorenz: {:g} ".format(
                                material.poles
                            )
                            for pole in range(material.poles):
                                dispersionstr += "{:g} {:g} {:g} ".format(
                                    material.deltaer[pole],
                                    material.tau[pole],
                                    material.alpha[pole],
                                )
                        elif "drude" in material.type:
                            dispersionstr = "#add_dispersion_drude: {:g} ".format(
                                material.poles
                            )
                            for pole in range(material.poles):
                                dispersionstr += "{:g} {:g} ".format(
                                    material.tau[pole], material.alpha[pole]
                                )
