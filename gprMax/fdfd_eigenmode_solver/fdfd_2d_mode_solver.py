...
if __name__ == "__main__":
    ...
    mu_r_xx = np.ones(shape, dtype=complex)
    ...
    solver.solve(reject_spurious=True)
    print("Effective indices:")
    for index, neff in enumerate(solver.complex_neff):
        print(f"  mode {index}: {neff}")
    print("PEC-neighbor energy scores:")
    for index in solver.accepted_candidate_indices:
        print(f"  candidate {index}: {solver.spurious_scores[index]:.6g}")
    solver.plot_e_fields("fdfd_modes_ex_ey.png")
    solver.plot_h_fields("fdfd_modes_hx_hy.png")
    # solver.plot_pec_component_masks("fdfd_modes_yee_sampled_masks.png")
