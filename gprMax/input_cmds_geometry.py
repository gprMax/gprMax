...
def process_geometrycmds(geometry, G):
    ...
    for object in tqdm(
        ...
        if tmp[0] == "#geometry_objects_read:":
            ...
            singlecmdsimport, multicmdsimport, geometryimport = check_cmd_names(
                materials, checkessential=False
            )
            ...
            if (
                round_value(dx_dy_dz[0] / G.dx) != 1
                or round_value(dx_dy_dz[1] / G.dy) != 1
                or round_value(dx_dy_dz[2] / G.dz) != 1
            ):
                raise CmdInputError(
                    "'"
                    + " ".join(tmp)
                    + "'"
                    + " requires the spatial resolution of the geometry objects file to match the spatial resolution of the model"
                )
            ...
            if np.amax(data) > len(materials) - 1:
                raise CmdInputError(
                    ...
                    + " ".join(tmp)
                    + "'"
                    + " found data value(s) ({}) in the geometry objects file greater than the maximum index for the specified materials ({})".format(
                        np.amax(data), len(materials) - 1
                    )
                )
            ...
            try:
                ...
                if G.messages:
                    tqdm.write(
                        "Geometry objects from file {} inserted at {:g}m, {:g}m, {:g}m, with corresponding materials file {}.".format(
                            geofile, xs * G.dx, ys * G.dy, zs * G.dz, matfile
                        )
                    )
            except KeyError:
                averaging = False
                ...
                if G.messages:
                    tqdm.write(
                        "Geometry objects from file (voxels only) {} inserted at {:g}m, {:g}m, {:g}m, with corresponding materials file {}.".format(
                            geofile, xs * G.dx, ys * G.dy, zs * G.dz, matfile
                        )
                    )
            ...
            if xs < 0 or xs > G.nx:
                raise CmdInputError(
                    ...
                    + " ".join(tmp)
                    + "'"
                    + " the lower x-coordinate {:g}m is not within the model domain".format(
                        xs * G.dx
                    )
                )
            if xf < 0 or xf > G.nx:
                raise CmdInputError(
                    ...
                    + " ".join(tmp)
                    + "'"
                    + " the upper x-coordinate {:g}m is not within the model domain".format(
                        xf * G.dx
                    )
                )
            if ys < 0 or ys > G.ny:
                raise CmdInputError(
                    ...
                    + " ".join(tmp)
                    + "'"
                    + " the lower y-coordinate {:g}m is not within the model domain".format(
                        ys * G.dy
                    )
                )
            if yf < 0 or yf > G.ny:
                raise CmdInputError(
                    ...
                    + " ".join(tmp)
                    + "'"
                    + " the upper y-coordinate {:g}m is not within the model domain".format(
                        yf * G.dy
                    )
                )
            if zs < 0 or zs > G.nz:
                raise CmdInputError(
                    ...
                    + " ".join(tmp)
                    + "'"
                    + " the lower z-coordinate {:g}m is not within the model domain".format(
                        zs * G.dz
                    )
                )
            if zf < 0 or zf > G.nz:
                raise CmdInputError(
                    ...
                    + " ".join(tmp)
                    + "'"
                    + " the upper z-coordinate {:g}m is not within the model domain".format(
                        zf * G.dz
                    )
                )
            if xs > xf or ys > yf or zs > zf:
                raise CmdInputError(
                    ...
                    + " ".join(tmp)
                    + "'"
                    + " the lower coordinates should be less than the upper coordinates"
                )
            ...
            if xs != xf:
                if ys != yf or zs != zf:
                    raise CmdInputError(
                        ...
                        + " ".join(tmp)
                        + "'"
                        + " the edge is not specified correctly"
                    )
                else:
                    for i in range(xs, xf):
                        build_edge_x(
                            i, ys, zs, material.numID, G.rigidE, G.rigidH, G.ID
                        )
            ...
            # y-orientated wire
                ...
                if xs != xf or zs != zf:
                    ...
                    for j in range(ys, yf):
                        build_edge_y(
                            xs, j, zs, material.numID, G.rigidE, G.rigidH, G.ID
                        )
            ...
            # z-orientated wire
                ...
                if xs != xf or ys != yf:
                    ...
                    for k in range(zs, zf):
                        build_edge_z(
                            xs, ys, k, material.numID, G.rigidE, G.rigidH, G.ID
                        )

            if G.messages:
                tqdm.write(
                    "Edge from {:g}m, {:g}m, {:g}m, to {:g}m, {:g}m, {:g}m of material {} created.".format(
                        xs * G.dx,
                        ys * G.dy,
                        zs * G.dz,
                        xf * G.dx,
            ...
            if xs == xf:
                if ys == yf or zs == zf:
                    raise CmdInputError(
                        ...
                        + " ".join(tmp)
                        + "'"
                        + " the plate is not specified correctly"
                    )
                ...
                raise CmdInputError(
                    "'" + " ".join(tmp) + "'" + " the plate is not specified correctly"
                )

            # Look up requested materials in existing list of material instances
            materials = [
                y for x in materialsrequested for y in G.materials if y.ID == x
            ]
            ...
            if len(materials) != len(materialsrequested):
                ...
                raise CmdInputError(
                    ...
                    + " ".join(tmp)
                    + "'"
                    + " material(s) {} do not exist".format(notfound)
                )
            ...
            if xs == xf:
                ...
                for j in range(ys, yf):
                    for k in range(zs, zf):
                        build_face_yz(
                            xs, j, k, numIDy, numIDz, G.rigidE, G.rigidH, G.ID
                        )
            ...
            # xz-plane plate
                ...
                for i in range(xs, xf):
                    for k in range(zs, zf):
                        build_face_xz(
                            i, ys, k, numIDx, numIDz, G.rigidE, G.rigidH, G.ID
                        )
            ...
            # xy-plane plate
                ...
                for i in range(xs, xf):
                    for j in range(ys, yf):
                        build_face_xy(
                            i, j, zs, numIDx, numIDy, G.rigidE, G.rigidH, G.ID
                        )

            if G.messages:
                tqdm.write(
                    "Plate from {:g}m, {:g}m, {:g}m, to {:g}m, {:g}m, {:g}m of material(s) {} created.".format(
                        xs * G.dx,
                        ys * G.dy,
                        zs * G.dz,
                        xf * G.dx,
            ...
            # Isotropic case with user specified averaging
                ...
                if tmp[12].lower() == "y":
                    ...
                    raise CmdInputError(
                        ...
                        + " ".join(tmp)
                        + "'"
                        + " requires averaging to be either y or n"
                    )
            ...
            if (
                ...
                raise CmdInputError(
                    ...
                    + " ".join(tmp)
                    + "'"
                    + " the one of the x-coordinates is not within the model domain"
                )
            if (
                y1 < 0
                or y2 < 0
                ...
                raise CmdInputError(
                    ...
                    + " ".join(tmp)
                    + "'"
                    + " the one of the y-coordinates is not within the model domain"
                )
            if (
                z1 < 0
                or z2 < 0
                ...
                raise CmdInputError(
                    ...
                    + " ".join(tmp)
                    + "'"
                    + " the one of the z-coordinates is not within the model domain"
                )
            if thickness < 0:
                raise CmdInputError(
                    "'"
                    + " ".join(tmp)
                    + "'"
                    + " requires a positive value for thickness"
                )
            ...
            # xy-plane triangle
                ...
                raise CmdInputError(
                    ...
                    + " ".join(tmp)
                    + "'"
                    + " the triangle is not specified correctly"
                )
            ...
            if G.messages:
                if thickness > 0:
                    ...
                    tqdm.write(
                        "Triangle with coordinates {:g}m {:g}m {:g}m, {:g}m {:g}m {:g}m, {:g}m {:g}m {:g}m and thickness {:g}m of material(s) {} created, dielectric smoothing is {}.".format(
                            x1,
                            y1,
                            z1,
                            x2,
                    ...
                    tqdm.write(
                        "Triangle with coordinates {:g}m {:g}m {:g}m, {:g}m {:g}m {:g}m, {:g}m {:g}m {:g}m of material(s) {} created.".format(
                            x1,
                            y1,
                            z1,
                            x2,
            ...
            if G.messages:
                if averaging:
                    ...
                    dielectricsmoothing = "off"
                tqdm.write(
                    "Box from {:g}m, {:g}m, {:g}m, to {:g}m, {:g}m, {:g}m of material(s) {} created, dielectric smoothing is {}.".format(
                        xs * G.dx,
                        ys * G.dy,
                        zs * G.dz,
                        xf * G.dx,
            ...
            if r <= 0:
                raise CmdInputError(
                    ...
                    + " ".join(tmp)
                    + "'"
                    + " the radius {:g} should be a positive value.".format(r)
                )
            ...
            if G.messages:
                if averaging:
                    ...
                    dielectricsmoothing = "off"
                tqdm.write(
                    "Cylinder with face centres {:g}m, {:g}m, {:g}m and {:g}m, {:g}m, {:g}m, with radius {:g}m, of material(s) {} created, dielectric smoothing is {}.".format(
                        x1,
                        y1,
                        z1,
                        x2,
            ...
            if normal != "x" and normal != "y" and normal != "z":
                raise CmdInputError(
                    ...
                    + " ".join(tmp)
                    + "'"
                    + " the normal direction must be either x, y or z."
                )
            if r <= 0:
                raise CmdInputError(
                    "'"
            ...
            if sectorstartangle < 0 or sectorangle <= 0:
                raise CmdInputError(
                    ...
                    + " ".join(tmp)
                    + "'"
                    + " the starting angle and sector angle should be a positive values."
                )
            if sectorstartangle >= 2 * np.pi or sectorangle >= 2 * np.pi:
                raise CmdInputError(
                    ...
                    + " ".join(tmp)
                    + "'"
                    + " the starting angle and sector angle must be less than 360 degrees."
                )
            ...
            if G.messages:
                if thickness > 0:
                    if averaging:
                        ...
                        dielectricsmoothing = "off"
                    tqdm.write(
                        "Cylindrical sector with centre {:g}m, {:g}m, radius {:g}m, starting angle {:.1f} degrees, sector angle {:.1f} degrees, thickness {:g}m, of material(s) {} created, dielectric smoothing is {}.".format(
                            ctr1,
                            ctr2,
                            r,
                            (sectorstartangle / (2 * np.pi)) * 360,
                            (sectorangle / (2 * np.pi)) * 360,
                            thickness,
                            ", ".join(materialsrequested),
                            dielectricsmoothing,
                        )
                    ...
                    tqdm.write(
                        "Cylindrical sector with centre {:g}m, {:g}m, radius {:g}m, starting angle {:.1f} degrees, sector angle {:.1f} degrees, of material(s) {} created.".format(
                            ctr1,
                            ctr2,
                            r,
                            (sectorstartangle / (2 * np.pi)) * 360,
            ...
            if G.messages:
                if averaging:
                    ...
                    dielectricsmoothing = "off"
                tqdm.write(
                    "Sphere with centre {:g}m, {:g}m, {:g}m, radius {:g}m, of material(s) {} created, dielectric smoothing is {}.".format(
                        xc * G.dx,
                        yc * G.dy,
                        zc * G.dz,
                        r,
            ...
            if float(tmp[7]) < 0:
                raise CmdInputError(
                    ...
                    + " ".join(tmp)
                    + "'"
                    + " requires a positive value for the fractal dimension"
                )
            if float(tmp[8]) < 0:
                raise CmdInputError(
                    "'"
                    + " ".join(tmp)
                    + "'"
                    + " requires a positive value for the fractal weighting in the x direction"
                )
            if float(tmp[9]) < 0:
                raise CmdInputError(
                    "'"
                    + " ".join(tmp)
                    + "'"
                    + " requires a positive value for the fractal weighting in the y direction"
                )
            if float(tmp[10]) < 0:
                raise CmdInputError(
                    "'"
                    + " ".join(tmp)
                    + "'"
                    + " requires a positive value for the fractal weighting in the z direction"
                )
            if round_value(tmp[11]) < 0:
                raise CmdInputError(
                    ...
                    + " ".join(tmp)
                    + "'"
                    + " requires a positive value for the number of bins"
                )
            ...
            if mixingmodel:
                if nbins == 1:
                    raise CmdInputError(
                        ...
                        + " ".join(tmp)
                        + "'"
                        + " must be used with more than one material from the mixing model."
                    )
                ...
                raise CmdInputError(
                    ...
                    + " ".join(tmp)
                    + "'"
                    + " mixing model or material with ID {} does not exist".format(
                        tmp[12]
                    )
                )
            ...
            if G.messages:
                if volume.averaging:
                    ...
                    dielectricsmoothing = "off"
                tqdm.write(
                    "Fractal box {} from {:g}m, {:g}m, {:g}m, to {:g}m, {:g}m, {:g}m with {}, fractal dimension {:g}, fractal weightings {:g}, {:g}, {:g}, fractal seeding {}, with {} material(s) created, dielectric smoothing is {}.".format(
                        volume.ID,
                        xs * G.dx,
                        ys * G.dy,
                        zs * G.dz,
                    ...
                    if tmp[12] == volume.ID:
                        ...
                        if float(tmp[8]) < 0:
                            raise CmdInputError(
                                ...
                                + " ".join(tmp)
                                + "'"
                                + " requires a positive value for the fractal weighting in the first direction of the surface"
                            )
                        if float(tmp[9]) < 0:
                            raise CmdInputError(
                                "'"
                                + " ".join(tmp)
                                + "'"
                                + " requires a positive value for the fractal weighting in the second direction of the surface"
                            )
                        ...
                        if xs == xf:
                            if ys == yf or zs == zf:
                                raise CmdInputError(
                                    ...
                                    + " ".join(tmp)
                                    + "'"
                                    + " dimensions are not specified correctly"
                                )
                            if xs != volume.xs and xs != volume.xf:
                                raise CmdInputError(
                                    ...
                                    + " ".join(tmp)
                                    + "'"
                                    + " can only be used on the external surfaces of a fractal box"
                                )
                            fractalrange = (
                                round_value(float(tmp[10]) / G.dx),
                                round_value(float(tmp[11]) / G.dx),
                            )
                            # xminus surface
                            if xs == volume.xs:
                                if fractalrange[0] < 0 or fractalrange[1] > volume.xf:
                                    raise CmdInputError(
                                        ...
                                        + " ".join(tmp)
                                        + "'"
                                        + " cannot apply fractal surface to fractal box as it would exceed either the upper coordinates of the fractal box or the domain in the x direction"
                                    )
                                requestedsurface = "xminus"
                            # xplus surface
                            elif xf == volume.xf:
                                if (
                                    fractalrange[0] < volume.xs
                                    or fractalrange[1] > G.nx
                                ):
                                    raise CmdInputError(
                                        "'"
                                        + " ".join(tmp)
                                        + "'"
                                        + " cannot apply fractal surface to fractal box as it would exceed either the lower coordinates of the fractal box or the domain in the x direction"
                                    )
                            ...
                            fractalrange = (
                                round_value(float(tmp[10]) / G.dy),
                                round_value(float(tmp[11]) / G.dy),
                            )
                            # yminus surface
                            if ys == volume.ys:
                                if fractalrange[0] < 0 or fractalrange[1] > volume.yf:
                                    raise CmdInputError(
                                        ...
                                        + " ".join(tmp)
                                        + "'"
                                        + " cannot apply fractal surface to fractal box as it would exceed either the upper coordinates of the fractal box or the domain in the y direction"
                                    )
                                requestedsurface = "yminus"
                            # yplus surface
                            elif yf == volume.yf:
                                if (
                                    fractalrange[0] < volume.ys
                                    or fractalrange[1] > G.ny
                                ):
                                    raise CmdInputError(
                                        "'"
                                        + " ".join(tmp)
                                        + "'"
                                        + " cannot apply fractal surface to fractal box as it would exceed either the lower coordinates of the fractal box or the domain in the y direction"
                                    )
                            ...
                            fractalrange = (
                                round_value(float(tmp[10]) / G.dz),
                                round_value(float(tmp[11]) / G.dz),
                            )
                            # zminus surface
                            if zs == volume.zs:
                            ...
                            # zplus surface
                                ...
                                if (
                                    fractalrange[0] < volume.zs
                                    or fractalrange[1] > G.nz
                                ):
                                    raise CmdInputError(
                                        "'"
                                        + " ".join(tmp)
                                        + "'"
                                        + " cannot apply fractal surface to fractal box as it would exceed either the lower coordinates of the fractal box or the domain in the z direction"
                                    )
                        ...
                        surface = FractalSurface(
                            xs, xf, ys, yf, zs, zf, float(tmp[7]), seed
                        )
                        ...
                        existingsurfaceIDs = [
                            x.surfaceID for x in volume.fractalsurfaces
                        ]
                        if surface.surfaceID in existingsurfaceIDs:
                            raise CmdInputError(
                                ...
                                + " ".join(tmp)
                                + "'"
                                + " has already been used on the {} surface".format(
                                    surface.surfaceID
                                )
                            )
                        ...
                        if G.messages:
                            tqdm.write(
                                "Fractal surface from {:g}m, {:g}m, {:g}m, to {:g}m, {:g}m, {:g}m with fractal dimension {:g}, fractal weightings {:g}, {:g}, fractal seeding {}, and range {:g}m to {:g}m, added to {}.".format(
                                    xs * G.dx,
                                    ys * G.dy,
                                    zs * G.dz,
                                    xf * G.dx,
                    ...
                    if tmp[8] == volume.ID:
                        ...
                        if depth <= 0:
                            raise CmdInputError(
                                ...
                                + " ".join(tmp)
                                + "'"
                                + " requires a positive value for the depth of water"
                            )
                        ...
                        surface = next(
                            (
                                x
                                for x in volume.fractalsurfaces
                                if x.surfaceID == requestedsurface
                            ),
                            None,
                        )
                        if not surface:
                            raise CmdInputError(
                                ...
                                + " ".join(tmp)
                                + "'"
                                + " specified surface {} does not have a rough surface applied".format(
                                    requestedsurface
                                )
                            )
                        ...
                        if (
                            ...
                            raise CmdInputError(
                                ...
                                + " ".join(tmp)
                                + "'"
                                + " requires a value for the depth of water that lies with the range of the requested surface roughness"
                            )
                        ...
                        if testwater:
                            raise CmdInputError(
                                ...
                                + " ".join(tmp)
                                + "'"
                                + " requires the time step for the model to be less than the relaxation time required to model water."
                            )

                        if G.messages:
                            tqdm.write(
                                "Water on surface from {:g}m, {:g}m, {:g}m, to {:g}m, {:g}m, {:g}m with depth {:g}m, added to {}.".format(
                                    xs * G.dx,
                                    ys * G.dy,
                                    zs * G.dz,
                                    xf * G.dx,
                    ...
                    if tmp[11] == volume.ID:
                        ...
                        if float(tmp[8]) < 0 or float(tmp[9]) < 0:
                            raise CmdInputError(
                                ...
                                + " ".join(tmp)
                                + "'"
                                + " requires a positive value for the minimum and maximum heights for grass blades"
                            )
                        ...
                        if xs == xf:
                            ...
                            if xs != volume.xs and xs != volume.xf:
                                raise CmdInputError(
                                    ...
                                    + " ".join(tmp)
                                    + "'"
                                    + " must specify external surfaces on a fractal box"
                                )
                            fractalrange = (
                                round_value(float(tmp[8]) / G.dx),
                                round_value(float(tmp[9]) / G.dx),
                            )
                            # xminus surface
                            if xs == volume.xs:
                                raise CmdInputError(
                                    ...
                                    + " ".join(tmp)
                                    + "'"
                                    + " grass can only be specified on surfaces in the positive axis direction"
                                )
                            # xplus surface
                            elif xf == volume.xf:
                                if fractalrange[1] > G.nx:
                                    raise CmdInputError(
                                        ...
                                        + " ".join(tmp)
                                        + "'"
                                        + " cannot apply grass to fractal box as it would exceed the domain size in the x direction"
                                    )
                                requestedsurface = "xplus"
                            ...
                            fractalrange = (
                                round_value(float(tmp[8]) / G.dy),
                                round_value(float(tmp[9]) / G.dy),
                            )
                            # yminus surface
                            if ys == volume.ys:
                                raise CmdInputError(
                            ...
                            # yplus surface
                                ...
                                if fractalrange[1] > G.ny:
                                    raise CmdInputError(
                                        ...
                                        + " ".join(tmp)
                                        + "'"
                                        + " cannot apply grass to fractal box as it would exceed the domain size in the y direction"
                                    )
                                requestedsurface = "yplus"
                            ...
                            fractalrange = (
                                round_value(float(tmp[8]) / G.dz),
                                round_value(float(tmp[9]) / G.dz),
                            )
                            # zminus surface
                            if zs == volume.zs:
                                raise CmdInputError(
                            ...
                            # zplus surface
                                ...
                                if fractalrange[1] > G.nz:
                                    raise CmdInputError(
                                        ...
                                        + " ".join(tmp)
                                        + "'"
                                        + " cannot apply grass to fractal box as it would exceed the domain size in the z direction"
                                    )
                                requestedsurface = "zplus"
                        ...
                        if (
                            ...
                            * surface.fractalsurface.shape[1]
                            ...
                            raise CmdInputError(
                                ...
                                + " ".join(tmp)
                                + "'"
                                + " the specified surface is not large enough for the number of grass blades/roots specified"
                            )

                        # Scale the distribution so that the summation is equal to one, i.e. a probability distribution
                        surface.fractalsurface = surface.fractalsurface / np.sum(
                            surface.fractalsurface
                        )
                        ...
                        for i in range(len(bladesindex[0])):
                            surface.fractalsurface[
                                bladesindex[0][i], bladesindex[1][i]
                            ] = R.randint(
                        ...
                        if testgrass:
                            raise CmdInputError(
                                ...
                                + " ".join(tmp)
                                + "'"
                                + " requires the time step for the model to be less than the relaxation time required to model grass."
                            )
                        ...
                        if G.messages:
                            tqdm.write(
                                "{} blades of grass on surface from {:g}m, {:g}m, {:g}m, to {:g}m, {:g}m, {:g}m with fractal dimension {:g}, fractal seeding {}, and range {:g}m to {:g}m, added to {}.".format(
                                    numblades,
                                    xs * G.dx,
                                    ys * G.dy,
                                    zs * G.dz,
            ...
            if volume.fractalsurfaces:
                ...
                if volume.nbins == 1:
                    volume.fractalvolume = np.ones(
                        (volume.nx, volume.ny, volume.nz), dtype=floattype
                    )
                    materialnumID = next(
                        x.numID for x in G.materials if x.ID == volume.operatingonID
                    )
                            ...
                            for j in range(surface.ys, surface.yf):
                                for k in range(surface.zs, surface.zf):
                                    if (
                                        ...
                                        volume.mask[
                                            i - volume.xs, j - volume.ys, k - volume.zs
                                        ] = 1
                                    elif (
                            ...
                            for j in range(surface.ys, surface.yf):
                                for k in range(surface.zs, surface.zf):
                                    if (
                                        ...
                                        height = 0
                                        for i in range(
                                            volume.xs, surface.fractalrange[1]
                                        ):
                                            if (
                                                i
                                                < surface.fractalsurface[
                            ...
                            for i in range(surface.xs, surface.xf):
                                for k in range(surface.zs, surface.zf):
                                    if (
                                        ...
                                        height = 0
                                        for j in range(
                                            volume.ys, surface.fractalrange[1]
                                        ):
                                            if (
                                                j
                                                < surface.fractalsurface[
                            ...
                            for i in range(surface.xs, surface.xf):
                                for j in range(surface.ys, surface.yf):
                                    if (
                                        ...
                                        height = 0
                                        for k in range(
                                            volume.zs, surface.fractalrange[1]
                                        ):
                                            if (
                                                k
                                                < surface.fractalsurface[
                ...
                if volume.nbins == 1:
                    raise CmdInputError(
                        ...
                        + " ".join(tmp)
                        + "'"
                        + " is being used with a single material and no modifications, therefore please use a #box command instead."
                    )
                else:
                    volume.generate_fractal_volume(G)
                    volume.fractalvolume += mixingmodel.startmaterialnum
