...
class PML(object):
    ...
    def initialise_field_arrays(self):
        ...
        if self.direction[0] == "x":
            self.EPhi1 = np.zeros(
                (len(self.CFS), self.nx + 1, self.ny, self.nz + 1), dtype=floattype
            )
            self.EPhi2 = np.zeros(
                (len(self.CFS), self.nx + 1, self.ny + 1, self.nz), dtype=floattype
            )
            self.HPhi1 = np.zeros(
                (len(self.CFS), self.nx, self.ny + 1, self.nz), dtype=floattype
            )
            self.HPhi2 = np.zeros(
                (len(self.CFS), self.nx, self.ny, self.nz + 1), dtype=floattype
            )
        elif self.direction[0] == "y":
            self.EPhi1 = np.zeros(
                (len(self.CFS), self.nx, self.ny + 1, self.nz + 1), dtype=floattype
            )
            self.EPhi2 = np.zeros(
                (len(self.CFS), self.nx + 1, self.ny + 1, self.nz), dtype=floattype
            )
            self.HPhi1 = np.zeros(
                (len(self.CFS), self.nx + 1, self.ny, self.nz), dtype=floattype
            )
            self.HPhi2 = np.zeros(
                (len(self.CFS), self.nx, self.ny, self.nz + 1), dtype=floattype
            )
    ...
    def calculate_update_coeffs(self, er, mr, G):
        ...
        for x, cfs in enumerate(self.CFS):
            ...
            if G.pmlformulation == "HORIPML":
                ...
                self.ERE[x, :] = (
                    (2 * e0 * Ekappa) - G.dt * (Ealpha * Ekappa + Esigma)
                ) / tmp
                ...
                self.HRE[x, :] = (
                    (2 * e0 * Hkappa) - G.dt * (Halpha * Hkappa + Hsigma)
                ) / tmp
    ...
    def update_electric(self, G):
        ...
        pmlmodule = (
            "gprMax.pml_updates.pml_updates_electric_" + G.pmlformulation + "_ext"
        )
        func = getattr(
    ...
    def update_magnetic(self, G):
        ...
        pmlmodule = (
            "gprMax.pml_updates.pml_updates_magnetic_" + G.pmlformulation + "_ext"
        )
        func = getattr(
            import_module(pmlmodule),
            "order" + str(len(self.CFS)) + "_" + self.direction,
    ...
    def gpu_set_blocks_per_grid(self, G):
        ...
        self.bpg = (
            int(
                np.ceil(
                    (
                        (self.EPhi1_gpu.shape[1] + 1)
                        * (self.EPhi1_gpu.shape[2] + 1)
                        * (self.EPhi1_gpu.shape[3] + 1)
                    )
                    / G.tpb[0]
                )
            ),
    ...
    def gpu_initialise_arrays(self):
        ...
        if self.direction[0] == "x":
            self.EPhi1_gpu = gpuarray.to_gpu(
                np.zeros(
                    (len(self.CFS), self.nx + 1, self.ny, self.nz + 1), dtype=floattype
                )
            )
            self.EPhi2_gpu = gpuarray.to_gpu(
                np.zeros(
                    (len(self.CFS), self.nx + 1, self.ny + 1, self.nz), dtype=floattype
                )
            )
            self.HPhi1_gpu = gpuarray.to_gpu(
                np.zeros(
                    (len(self.CFS), self.nx, self.ny + 1, self.nz), dtype=floattype
                )
            )
            self.HPhi2_gpu = gpuarray.to_gpu(
                np.zeros(
                    (len(self.CFS), self.nx, self.ny, self.nz + 1), dtype=floattype
                )
            )
    ...
    def gpu_get_update_funcs(self, kernelselectric, kernelsmagnetic):
        ...
        self.update_electric_gpu = kernelselectric.get_function(
            "order" + str(len(self.CFS)) + "_" + self.direction
        )
        self.update_magnetic_gpu = kernelsmagnetic.get_function(
            "order" + str(len(self.CFS)) + "_" + self.direction
        )
