...
def process_singlecmds(singlecmds, G):
    ...
    if sys.platform == "darwin":
        os.environ["OMP_WAIT_POLICY"] = (
            "ACTIVE"  # Should waiting threads consume CPU power (can drastically effect performance)
        )
    os.environ["OMP_DYNAMIC"] = (
        "FALSE"  # Number of threads may be adjusted by the run time environment to best utilize system resources
    )
    ...
    if G.nthreads > G.hostinfo["physicalcores"]:
        print(
            Fore.RED
            + "WARNING: You have specified more threads ({}) than available physical CPU cores ({}). This may lead to degraded performance.".format(
                G.nthreads, hostinfo["physicalcores"]
            )
            + Style.RESET_ALL
        )
    ...
    if tmp[0] <= 0:
        raise CmdInputError(
            cmd + " requires the x-direction spatial step to be greater than zero"
        )
    if tmp[1] <= 0:
        raise CmdInputError(
            cmd + " requires the y-direction spatial step to be greater than zero"
        )
    if tmp[2] <= 0:
        raise CmdInputError(
            cmd + " requires the z-direction spatial step to be greater than zero"
        )
    G.dx = tmp[0]
    G.dy = tmp[1]
    G.dz = tmp[2]
    ...
    if G.messages:
        print(
            "Domain size: {:g} x {:g} x {:g}m ({:d} x {:d} x {:d} = {:g} cells)".format(
    ...
    if G.messages:
        print(
            "Time window: {:g} secs ({} iterations)".format(G.timewindow, G.iterations)
        )
    ...
    if (
        2 * G.pmlthickness["x0"] >= G.nx
        or 2 * G.pmlthickness["y0"] >= G.ny
        or 2 * G.pmlthickness["z0"] >= G.nz
        or 2 * G.pmlthickness["xmax"] >= G.nx
        or 2 * G.pmlthickness["ymax"] >= G.ny
        or 2 * G.pmlthickness["zmax"] >= G.nz
    ):
    ...
    if singlecmds[cmd] is not None:
        ...
        if G.messages:
            print(
                "Simple sources will step {:g}m, {:g}m, {:g}m for each model run.".format(
                    G.srcsteps[0] * G.dx, G.srcsteps[1] * G.dy, G.srcsteps[2] * G.dz
                )
            )
        ...
        if G.messages:
            print(
                "All receivers will step {:g}m, {:g}m, {:g}m for each model run.".format(
                    G.rxsteps[0] * G.dx, G.rxsteps[1] * G.dy, G.rxsteps[2] * G.dz
                )
            )
    ...
    if singlecmds[cmd] is not None:
        ...
        for waveform in range(len(waveformIDs)):
            if any(x.ID == waveformIDs[waveform] for x in G.waveforms):
                raise CmdInputError(
                    "Waveform with ID {} already exists".format(waveformIDs[waveform])
                )
            w = Waveform()
            ...
            if G.messages:
                print(
                    "User waveform {} created using {} and, if required, interpolation parameters (kind: {}, fill value: {}).".format(
                        w.ID, timestr, kwargs["kind"], kwargs["fill_value"]
                    )
                )
