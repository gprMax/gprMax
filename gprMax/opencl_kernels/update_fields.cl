...
__kernel void update_e_dispersive_A(int NX, int NY, int NZ, int MAXPOLES, __global const {{COMPLEX-}}_t* restrict updatecoeffsdispersive, __global {{COMPLEX-}}_t *Tx, __global {{COMPLEX-}}_t *Ty, __global {{COMPLEX-}}_t *Tz, __global const unsigned int* restrict ID, __global {{REAL}} *Ex, __global {{REAL}} *Ey, __global {{REAL}} *Ez, __global const {{REAL}}* restrict Hx, __global const {{REAL}}* restrict Hy, __global const {{REAL}}* restrict Hz){
    ...
    if ((NY != 1 || NZ != 1) && i >= 0 && i < NX && j > 0 && j < NY && k > 0 && k < NZ) {
        ...
        for (int pole = 0; pole < MAXPOLES; pole++) {
            ...
            Tx[INDEX4D_T(pole,i_T,j_T,k_T)] = cfloat_add(cfloat_mul(updatecoeffsdispersive[INDEX2D_MATDISP(materialEx,1+(pole*3))], Tx[INDEX4D_T(pole,i_T,j_T,k_T)]), cfloat_mulr(updatecoeffsdispersive[INDEX2D_MATDISP(materialEx,2+(pole*3))], Ex[INDEX3D_FIELDS(i,j,k)]));
        }
    ...
    if ((NX != 1 || NZ != 1) && i > 0 && i < NX && j >= 0 && j < NY && k > 0 && k < NZ) {
        ...
        for (int pole = 0; pole < MAXPOLES; pole++) {
            ...
            Ty[INDEX4D_T(pole,i_T,j_T,k_T)] = cfloat_add(cfloat_mul(updatecoeffsdispersive[INDEX2D_MATDISP(materialEy,1+(pole*3))], Ty[INDEX4D_T(pole,i_T,j_T,k_T)]), cfloat_mulr(updatecoeffsdispersive[INDEX2D_MATDISP(materialEy,2+(pole*3))], Ey[INDEX3D_FIELDS(i,j,k)]));
        }
    ...
    if ((NX != 1 || NY != 1) && i > 0 && i < NX && j > 0 && j < NY && k >= 0 && k < NZ) {
        ...
        for (int pole = 0; pole < MAXPOLES; pole++) {
            ...
            Tz[INDEX4D_T(pole,i_T,j_T,k_T)] = cfloat_add(cfloat_mul(updatecoeffsdispersive[INDEX2D_MATDISP(materialEz,1+(pole*3))], Tz[INDEX4D_T(pole,i_T,j_T,k_T)]), cfloat_mulr(updatecoeffsdispersive[INDEX2D_MATDISP(materialEz,2+(pole*3))], Ez[INDEX3D_FIELDS(i,j,k)]));
        }
...
__kernel void update_e_dispersive_B(int NX, int NY, int NZ, int MAXPOLES, __global const {{COMPLEX-}}_t* restrict updatecoeffsdispersive, __global {{COMPLEX-}}_t *Tx, __global {{COMPLEX-}}_t *Ty, __global {{COMPLEX-}}_t *Tz, __global const unsigned int* restrict ID, __global const {{REAL}}* restrict Ex, __global const {{REAL}}* restrict Ey, __global const {{REAL}}* restrict Ez){
    ...
    if ((NY != 1 || NZ != 1) && i >= 0 && i < NX && j > 0 && j < NY && k > 0 && k < NZ) {
        int materialEx = ID[INDEX4D_ID(0,i_ID,j_ID,k_ID)];
        for (int pole = 0; pole < MAXPOLES; pole++) {
            Tx[INDEX4D_T(pole,i_T,j_T,k_T)] = cfloat_sub(Tx[INDEX4D_T(pole,i_T,j_T,k_T)], cfloat_mulr(updatecoeffsdispersive[INDEX2D_MATDISP(materialEx,2+(pole*3))], Ex[INDEX3D_FIELDS(i,j,k)]));
        }
    }
    ...
    if ((NX != 1 || NZ != 1) && i > 0 && i < NX && j >= 0 && j < NY && k > 0 && k < NZ) {
        int materialEy = ID[INDEX4D_ID(1,i_ID,j_ID,k_ID)];
        for (int pole = 0; pole < MAXPOLES; pole++) {
            Ty[INDEX4D_T(pole,i_T,j_T,k_T)] = cfloat_sub(Ty[INDEX4D_T(pole,i_T,j_T,k_T)], cfloat_mulr(updatecoeffsdispersive[INDEX2D_MATDISP(materialEy,2+(pole*3))], Ey[INDEX3D_FIELDS(i,j,k)]));
        }
    }
    ...
    if ((NX != 1 || NY != 1) && i > 0 && i < NX && j > 0 && j < NY && k >= 0 && k < NZ) {
        int materialEz = ID[INDEX4D_ID(2,i_ID,j_ID,k_ID)];
        for (int pole = 0; pole < MAXPOLES; pole++) {
            Tz[INDEX4D_T(pole,i_T,j_T,k_T)] = cfloat_sub(Tz[INDEX4D_T(pole,i_T,j_T,k_T)], cfloat_mulr(updatecoeffsdispersive[INDEX2D_MATDISP(materialEz,2+(pole*3))], Ez[INDEX3D_FIELDS(i,j,k)]));
        }
    }
}
