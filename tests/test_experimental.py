...
parser = argparse.ArgumentParser(
    ...
    usage="cd gprMax; python -m tests.test_compare_experimental modelfile realfile output",
)
...
if args.output[0] not in availablecomponents:
    raise CmdInputError(
        "{} output requested to plot, but the available output for receiver 1 is {}".format(
            args.output[0], ", ".join(availablecomponents)
        )
    )
...
fig, ax = plt.subplots(
    num=args.modelfile + " versus " + args.realfile,
    figsize=(20, 10),
    facecolor="w",
    edgecolor="w",
)
...
savename = (
    os.path.abspath(os.path.dirname(args.modelfile))
    + os.sep
    + os.path.splitext(os.path.split(args.modelfile)[1])[0]
    + "_vs_"
    + os.path.splitext(os.path.split(args.realfile)[1])[0]
)
