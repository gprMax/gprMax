...
class TestDiscretisationAnyBug:
    ...
    def test_negative_step_now_raises(self, stub_model):
        """Upstream fixed: negative discretisation now correctly raises ValueError."""
...
class TestPMLFormulation:
    def test_constructor_stores_attribute_and_kwargs(self):
        ...
        assert p.kwargs["id"] is None  # upstream added id field

    def test_order_and_hash(self):
        p = PMLFormulation("HORIPML")
        assert p.order == 18  # upstream renumbered
    ...
    def test_build_accepts_known_formulations(self, stub_grid, formulation):
        PMLFormulation(formulation).build(stub_grid)
        assert stub_grid.pmls["formulation"] == formulation

    def test_build_rejects_unknown_formulation(self, stub_grid):
        with pytest.raises(ValueError):
            PMLFormulation("UNKNOWN").build(stub_grid)
