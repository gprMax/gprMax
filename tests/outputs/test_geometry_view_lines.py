...
def _total_lines(nx, ny, nz):
    """Total number of cell-edge lines for a grid of size (nx, ny, nz).

    x-directed edges: nx * (ny+1) * (nz+1)
...
class TestLinePropertiesKernel:
    ...
    def test_a_single_cell_gives_twelve_lines(self):
        """Expects all boundary edges for one cell: 4 x-lines + 4 y-lines + 4 z-lines."""
        nx, ny, nz = 1, 1, 1
        n = _total_lines(nx, ny, nz)
        ID = np.zeros((6, nx + 1, ny + 1, nz + 1), dtype=np.uint32)
        connectivity, material_data = get_line_properties(n, nx, ny, nz, ID)
        assert len(material_data) == n  # 12
        assert len(connectivity) == 2 * n  # 24

    def test_a_single_cell_connectivity_is_hand_computable(self):
        """For ``nx=ny=nz=1`` the strides are ``x_step=4``, ``y_step=2``,
        ``z_step=1``.

        x-lines: (i=0, j=0..1, k=0..1) → edges 0→4, 1→5, 2→6, 3→7
        y-lines: (i=0..1, j=0, k=0..1) → edges 0→2, 1→3, 4→6, 5→7
        z-lines: (i=0..1, j=0..1, k=0) → edges 0→1, 2→3, 4→5, 6→7
        """
        nx, ny, nz = 1, 1, 1
        n = _total_lines(nx, ny, nz)
        ID = np.zeros((6, nx + 1, ny + 1, nz + 1), dtype=np.uint32)
    ...
    def test_two_cells_along_x_skip_the_far_edge_points(self):
        """For ``nx=2, ny=1, nz=1`` the correct connectivity.

        x-step = (1+1)*(1+1) = 4, y-step = 2, z-step = 1.
        x-lines: 2*2*2 = 8 edges
        y-lines: 3*1*2 = 6 edges
        z-lines: 3*2*1 = 6 edges
        Total: 20 edges.
        """
        nx, ny, nz = 2, 1, 1
        n = _total_lines(nx, ny, nz)
        ID = np.zeros((6, nx + 1, ny + 1, nz + 1), dtype=np.uint32)
        connectivity, _ = get_line_properties(n, nx, ny, nz, ID)
        assert len(connectivity) == 2 * n  # 40

    def test_line_count_is_the_formula_for_any_grid(self):
        """Expects lines = nx*(ny+1)*(nz+1) + ny*(nx+1)*(nz+1) + nz*(nx+1)*(ny+1)."""
        nx, ny, nz = 3, 2, 4
        n = _total_lines(nx, ny, nz)
        ID = np.zeros((6, nx + 1, ny + 1, nz + 1), dtype=np.uint32)
        _, material_data = get_line_properties(n, nx, ny, nz, ID)
        assert len(material_data) == n
    ...
    def test_each_line_takes_its_own_id_component(self):
        ...
        ID[2, 0, 0, 0] = 9
        _, material_data = get_line_properties(n, nx, ny, nz, ID)
        # The first x-edge, first y-edge, and first z-edge
        assert material_data[0] == 7
        assert material_data[nx * (ny + 1) * (nz + 1)] == 8  # first y-edge
        first_z = nx * (ny + 1) * (nz + 1) + ny * (nx + 1) * (nz + 1)
        assert material_data[first_z] == 9  # first z-edge
    ...
    def test_higher_id_components_are_ignored(self):
        ...
        ID[3:, ...] = 99
        _, material_data = get_line_properties(n, nx, ny, nz, ID)
        assert material_data.tolist() == [0] * n
    ...
    def test_cells_are_visited_in_x_then_y_then_z_order(self):
        ...
        ID[0, 0, 0, 1] = 2
        _, material_data = get_line_properties(n, nx, ny, nz, ID)
        # First x-edge of cell (0,0,0) vs cell (0,0,1)
        assert material_data[0] == 1
        # Next cell along z: index jumps by (nz+1) within the x-edge loop
        assert material_data[1] == 2
...
class TestPrepVtk:
    ...
    def test_line_count_follows_the_formula(self, make_line_view):
        """Expects lines = nx*(ny+1)*(nz+1) + ny*(nx+1)*(nz+1) + nz*(nx+1)*(ny+1)."""
        view = make_line_view(stop=(2, 2, 2), nx=8, ny=8, nz=8)
        expected = _total_lines(2, 2, 2)
        assert len(view.cell_types) == expected  # 54, not 3*2*2*2=24
    ...
    def test_cell_offsets_step_by_two(self, make_line_view):
        ...
        n = _total_lines(1, 1, 1)
        assert view.cell_offsets.tolist() == list(range(0, 2 * n + 2, 2))
...
class TestWriteVtk:
    ...
    def test_writes_the_connectivity(self, make_line_view, read_h5):
        ...
        view.write_vtk()
        _, data = read_h5(view.filename)
        n = _total_lines(2, 2, 2)
        assert data["VTKHDF/Connectivity"].shape == (2 * n,)
    ...
    def test_writes_the_cell_offsets(self, make_line_view, read_h5):
        ...
        _, data = read_h5(view.filename)
        n = _total_lines(2, 2, 2)
        assert data["VTKHDF/Offsets"].shape == (n + 1,)
    ...
    def test_writes_the_cell_types(self, make_line_view, read_h5):
        ...
        _, data = read_h5(view.filename)
        n = _total_lines(2, 2, 2)
        assert data["VTKHDF/Types"].shape == (n,)
    ...
    def test_declares_the_counts(self, make_line_view, read_h5):
        ...
        _, data = read_h5(view.filename)
        n = _total_lines(2, 2, 2)
        assert int(np.ravel(data["VTKHDF/NumberOfCells"])[0]) == n
        assert int(np.ravel(data["VTKHDF/NumberOfPoints"])[0]) == 27
        assert int(np.ravel(data["VTKHDF/NumberOfConnectivityIds"])[0]) == 2 * n
