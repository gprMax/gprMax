...
class TestRxAllowableOutputs:
    ...
    def test_allowableoutputs_dev_now_includes_currents(self):
        """Upstream added Ix/Iy/Iz currents to the device-allowable outputs."""
        assert "Ix" in Rx.allowableoutputs_dev
        assert "Iy" in Rx.allowableoutputs_dev
        assert "Iz" in Rx.allowableoutputs_dev
...
class TestDtohRxArrayHostPath:
    ...
    def test_rxs_with_unmatched_coords_now_raises(self, fake_grid):
...
class TestDtohRxArrayLoopBoundBug:
    """Upstream fixed the inner-loop bound bug at ``receivers.py``.
    Now raises RuntimeError for count mismatch instead of IndexError."""

    def test_more_rxs_than_rxcoords_raises_runtimeerror(self, fake_grid):
