...
for i, model in enumerate(testmodels):
    ...
    if model == "hertzian_dipole_fs_analytical":
        ...
        datatest = np.zeros(
            (filetest.attrs["Iterations"], len(outputstest)), dtype=floattype
        )
        ...
        fileref = h5py.File(
            os.path.join(basepath, model + os.path.sep + model + "_ref.out"), "r"
        )
        filetest = h5py.File(
            os.path.join(basepath, model + os.path.sep + model + ".out"), "r"
        )
        ...
        if outputsref != outputstest:
            raise GeneralError(
                "Field output components do not match reference solution"
            )
        ...
        if filetest[path + outputstest[0]].dtype != fileref[path + outputsref[0]].dtype:
            print(
                Fore.RED
                + "WARNING: Type of floating point number in test model ({}) does not match type in reference solution ({})\n".format(
                    filetest[path + outputstest[0]].dtype,
                    fileref[path + outputsref[0]].dtype,
                )
                + Style.RESET_ALL
            )
        ...
        # Arrays for storing field data
        dataref = np.zeros(
            (fileref.attrs["Iterations"], len(outputsref)), dtype=floattyperef
        )
        datatest = np.zeros(
            (filetest.attrs["Iterations"], len(outputstest)), dtype=floattypetest
        )
    ...
    fig1.savefig(
        savename + ".png", dpi=150, format="png", bbox_inches="tight", pad_inches=0.1
    )
    fig2.savefig(
        savename + "_diffs.png",
        dpi=150,
...
for name, data in sorted(testresults.items()):
    if "analytical" in name:
        print(
            Fore.CYAN
            + "Test '{}.in' using v.{} compared to analytical solution. Max difference {:.2f}dB.".format(
                name, data["Test version"], data["Max diff"]
            )
            + Style.RESET_ALL
        )
    else:
        print(
            Fore.CYAN
            + "Test '{}.in' using v.{} compared to reference solution using v.{}. Max difference {:.2f}dB.".format(
                name, data["Test version"], data["Ref version"], data["Max diff"]
            )
            + Style.RESET_ALL
        )
