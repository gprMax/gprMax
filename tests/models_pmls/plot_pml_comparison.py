...
for x, model in enumerate(testmodels):
    ...
    timeref = np.zeros((fileref.attrs["Iterations"]), dtype=floattyperef)
    timeref = np.linspace(
        0, (fileref.attrs["Iterations"] - 1), num=fileref.attrs["Iterations"]
    )
    timetest = np.zeros((filetest.attrs["Iterations"]), dtype=floattypetest)
    timetest = np.linspace(
        0, (filetest.attrs["Iterations"] - 1), num=filetest.attrs["Iterations"]
    )
...
fig.savefig(
    basepath + ".pdf", dpi=None, format="pdf", bbox_inches="tight", pad_inches=0.1
)
