...
class TestValidation:
    ...
    def test_combined_accelerators_are_not_actually_rejected(self, make_sim_config):
        """The guard against selecting two accelerators never fires.

        ``[args.gpu, args.opencl, args.metal].count(True) > 1`` counts values
        equal to ``True``. The CLI parses ``-gpu`` with
        ``action="append", nargs="*"``, producing a *list*, and the API
        passes lists too — so ``count(True)`` is zero and the check passes.

        The three device branches that follow are independent ``if``
        statements rather than a chain, so the last one wins: asking for both
        CUDA and OpenCL silently yields ``solver == "opencl"`` with a device
        dictionary built for it. No error, no warning.

        Written up in ``notes/bugs/config-combined-accelerator-guard.md``.
        """
        sim_config = make_sim_config(gpu=[0], opencl=[0])

        assert sim_config.general["solver"] == "opencl"
...
class TestModelStartAndEnd:
    ...
    def test_a_restart_index_of_zero_is_treated_as_absent(self, make_sim_config):
        ...
        """
        assert make_sim_config(i=0, n=3).model_start == 0
