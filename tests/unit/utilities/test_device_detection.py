...
class TestDetectMetal:
    ...
    def test_no_hardware_still_produces_an_entry(self, fake_module):
        ...
        The return value is stored unconditionally, so the dictionary is
        non-empty and truthy while containing nothing usable. Anything that
        later tests ``if devs:`` is misled, and the failure surfaces as an
        ``AttributeError`` on ``None`` inside ``print_metal_info``. Pinned as
...
class TestPrintCudaInfo:
    ...
    def test_one_line_per_device(self, caplog):
        ...
        assert len(caplog.records) == 4  # heading, debug version, two devices
