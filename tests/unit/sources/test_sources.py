...
class TestHtodSrcArraysVoltageSourceDeadCodeBug:
    ...
    def test_hard_voltage_source_srcwaves_currently_uses_halfdt(
        ...
        # If the bug were fixed, hard source would ship wholedt (10/20/30).
        # Today the unconditional override at line 449 ships halfdt.
        assert np.array_equal(srcwaves[0], src.waveformvalues_halfdt)
...
class TestDiscretePlaneWaveInit:
    ...
    def test_default_scalar_fields(self):
        dpw = DiscretePlaneWave(G=None)
        assert dpw.materialID == 1
        assert dpw.pml_cells == 20
        assert dpw.buffercells_axial == 5
        assert dpw.axial == 0
        assert dpw.speed == c  # config.c → scipy.constants.c via the fixture
