...
class TestWriteReceivers:
    ...
    def test_the_sort_mutates_the_grids_receiver_list(
        ...
        """Expects ``grid.rxs`` itself to be reordered — the writer sorts in
        place, so a caller holding the list sees it change as a side effect of
        writing output."""
        import h5py
