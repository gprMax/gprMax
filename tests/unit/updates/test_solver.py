...
class MinimalRecordingUpdates(Updates):
    ...
    # Overridden purely so the recorder can see them. A backend that does
...
class TestIterationOrder:
    ...
    def test_there_are_eleven_steps_in_an_iteration(self, recording_updates):
        Solver(recording_updates).solve(range(1))
...
class TestBackendSpecificSteps:
    ...
    def test_a_non_cpu_backend_still_runs_the_nine_shared_steps(self):
        ...
        assert body == [
            ...
            if not step.startswith("update_plane_waves")
        ]
