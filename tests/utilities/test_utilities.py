...
class TestRoundInt:
    ...
    def test_a_numpy_float_is_rejected(self):
        ...
        """
        # Upstream fixed: numpy float no longer raises TypeError.
        assert round_int(np.float64(2.7)) == 3
...
class TestFftPower:
    ...
    def test_an_all_zero_waveform_does_not_produce_infinities(self):
        """Upstream: all-zero waveform produces all ``-inf`` (line 229).

        ``log10(0)`` is ``-inf``; the function no longer replaces non-finite
        values. Spectral nulls are intentionally kept as ``-inf`` rather than
        being turned into apparent 0 dB peaks.
        """
    ...
    def test_a_waveform_with_a_zero_bin_does_not_produce_infinities(self):
        """Upstream: spectral nulls are intentionally left as ``-inf``.

        A real signal can still have an exactly zero frequency component.
        The function intentionally leaves ``-inf`` at null bins rather than
        replacing them. For ``[1,-1] * 16``, only the Nyquist bin is non-zero.
        """
        waveform = np.array([1.0, -1.0] * 16)

        _, power = fft_power(waveform, 1e-9)

        # Nyquist is the only non-zero component (0 dB); the rest are -inf.
        assert np.isfinite(power[len(power) // 2])  # Nyquist bin
