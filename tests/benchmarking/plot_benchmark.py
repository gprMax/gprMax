...
parser = argparse.ArgumentParser(
    description="Plots execution times and speedup factors from benchmarking models run with different numbers of CPU (OpenMP) threads. Can also benchmark GPU(s) if required. Results are read from a NumPy archive.",
    usage="cd gprMax; python -m tests.benchmarking.plot_benchmark numpyfile",
)
parser.add_argument("baseresult", help="name of NumPy archive file including path")
parser.add_argument(
    "--otherresults",
    default=None,
    help="list of NumPy archives file including path",
    nargs="+",
)
...
try:
    ...
    hyperthreading = (
        ", {} cores with Hyper-Threading".format(hostinfo["logicalcores"])
...
cells = np.array(
    [baseresult["numcells"][0]]
)  # Length of cubic model side for cells per second metric
...
if baseresult["cputhreads"].size != 0:
    for i in range(len(baseresult["cputhreads"])):
        print(
            "{} CPU (OpenMP) thread(s): {:g} s".format(
                baseresult["cputhreads"][i], baseresult["cputimes"][i]
            )
        )
    cpucellspersec = np.array(
        [
            (
                baseresult["numcells"][0]
                * baseresult["numcells"][1]
                * baseresult["numcells"][2]
                * baseresult["iterations"]
            )
            / baseresult["cputimes"][0]
        ]
    )
...
if args.otherresults is not None:
    for i, result in enumerate(args.otherresults):
        ...
        print("\nModel: {}".format(result))
        cells = np.append(
            cells, otherresults[i]["numcells"][0]
        )  # Length of cubic model side for cells per second metric
        ...
        if otherresults[i]["cputhreads"].size != 0:
            for thread in range(len(otherresults[i]["cputhreads"])):
                print(
                    "{} CPU (OpenMP) thread(s): {:g} s".format(
                        otherresults[i]["cputhreads"][thread],
                        otherresults[i]["cputimes"][thread],
                    )
                )
            cpucellspersec = np.append(
                cpucellspersec,
                (
                    otherresults[i]["numcells"][0]
                    * otherresults[i]["numcells"][1]
                    * otherresults[i]["numcells"][2]
                    * otherresults[i]["iterations"]
                )
                / otherresults[i]["cputimes"][0],
            )
        ...
        if othergpuIDs:
            ...
            for j in range(len(othergpuIDs)):
                print(
                    "NVIDIA {}: {:g} s".format(
                        othergpuIDs[j], otherresults[i]["gputimes"][j]
                    )
                )
                gpucellspersec[j, i + 1] = (
                    otherresults[i]["numcells"][0]
                    * otherresults[i]["numcells"][1]
                    * otherresults[i]["numcells"][2]
                    * otherresults[i]["iterations"]
                ) / otherresults[i]["gputimes"][j]
...
if baseresult["cputhreads"].size != 0:
    ...
    ax.plot(
        baseresult["cputhreads"],
        baseresult["cputimes"][-1] / baseresult["cputimes"],
        color=next(colors),
        marker=markers[0],
        markeredgecolor="none",
        ms=8,
    ...
    if args.otherresults is not None:
        for i, result in enumerate(otherresults):
            ax.plot(
                result["cputhreads"],
                result["cputimes"][-1] / result["cputimes"],
                color=next(colors),
                marker=markers[0],
                markeredgecolor="none",
                ms=8,
...
if gpuIDs:
    ...
    for i in range(gpucellspersec.shape[0]):
        gpu = ax.bar(
            cells + positions[i] * barwidth,
            gpucellspersec[i, :] / 1e6,
            barwidth,
            color=next(colors),
            edgecolor="none",
            label="NVIDIA " + gpuIDs[i],
        )
        autolabel(gpu)
...
fig.savefig(
    os.path.join(
        os.path.dirname(args.baseresult), machineID.replace(" ", "_") + ".png"
    ),
    dpi=150,
    format="png",
    bbox_inches="tight",
