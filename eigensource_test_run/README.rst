...
``broadband_comparison``
    Three matched single-solve versus seven-solve comparisons, all driven by
    broadband Gaussian-derivative pulses rather than continuous sine waves:

    - The comparison-root pair is a 2D TM dielectric slab at 1 GHz. Its
      seven anchors are 0.1, 0.14, 0.2, 0.32, 0.6, 1.5, and 2 GHz. The lowest
      anchor puts the fundamental guided mode just above the light line, and
      2.4 m of free space on each side of the core lets its long evanescent
      tail decay before the PML. Receiver lines integrate
      ``Sx = -Ez*Hy``.
    - ``2d_te`` is a 2D TE dielectric slab at 1 GHz. Its anchors are 0.5,
      0.6, 0.75, 1, 1.5, 2.5, and 5 GHz, all on the guided branch. Receiver
      lines integrate ``Sx = Ey*Hz``. This case also checks that broadband
      normalization uses only the live 2D TE layer rather than averaging in
      the synthetic invariant-grid layers.
    - ``3d`` is a rectangular dielectric channel at 0.5 GHz. Its unequal
      transverse core dimensions avoid a square-guide polarization
      degeneracy. The seven anchors are 0.5, 0.6, 0.7, 0.85, 1.1, 1.6, and
      2.5 GHz. Receiver planes integrate
      ``Sx = Ey*Hz - Ez*Hy`` over both transverse coordinates.

    Each pair contains ``single_frequency`` and ``seven_frequency`` cases
    with identical geometry, waveform, receivers, and snapshot times. The
    one-frequency case is the control; the seven-frequency case interpolates
    the complex modal fields. The 2D cases retain four ``xy`` snapshots, and
    the 3D cases retain four ``xy`` plus four ``xz`` snapshots.
...
``broadband_comparison[/2d_te|/3d]/normalized_forward_backward_power.png``
    Forward and backward power histories for each matched single-solve and
    seven-solve guide, normalized by the peak power of the unit-power modal
    source. Legends also report energy normalized by the source-pulse energy.

``broadband_comparison[/2d_te|/3d]/normalized_forward_backward_power_spectrum_db.png``
    Forward and backward FFT-domain Poynting power in dB, divided bin-by-bin
    by the source waveform power spectrum. Only frequencies where the source
    amplitude spectrum is at least ``1e-3`` of its peak are plotted.

``broadband_comparison[/2d_te|/3d]/unnormalized_forward_backward_power_spectrum_db.png``
    Forward and backward FFT-domain Poynting power on their common raw dB
    scale, without division by the source spectrum. A dashed source-power
    spectrum is included in both panels as the baseline.
...
The broadband comparison time- and frequency-domain power plots can also be
regenerated directly:

.. code-block:: powershell

   python eigensource_test_run\broadband_comparison\plot_power_comparison.py
