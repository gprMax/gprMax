...
def plot_comparison(root: Path) -> Path:
    ...
    for case, label in (
        ("broadband", "multi-anchor broadband"),
        ("single_frequency", "single 3 GHz profile"),
    ):
    ...
    axis.set(
        ...
        title="Broadband versus single-frequency modal profiles",
    )
    axis.grid(True, alpha=0.3)
    axis.legend()
    output = root / "broadband_vs_single_frequency_s21.png"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Compare broadband and single-profile modal S21.")
