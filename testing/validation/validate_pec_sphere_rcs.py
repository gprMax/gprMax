"""Validate broadband PEC-sphere backscatter RCS against the Mie series.

The model uses a discrete plane wave, a closed equivalent-current NTFF
surface, and the same fine 0.5 mm sphere representation as the documented
gprMax example. HDF5 data are treated as a reusable cache; the retained
validation evidence is the comparison plot, CSV data, summary, and report.

Examples::

    python -m testing.validation.validate_pec_sphere_rcs --gpu 0
    python -m testing.validation.validate_pec_sphere_rcs --reuse
"""
...
def analyse(frequencies, simulated):
    ...
    return {
        ...
        "wavelength_cells": c / (frequencies * DL),
...
def _plot(path, result):
    ...
    axes[0].set(
        ...
        title=f"PEC sphere monostatic RCS: radius {RADIUS * 1e3:g} mm",
    )
    axes[1].set(xlabel=r"Electrical size, $ka=2\pi f a/c$", ylabel="gprMax - Mie [dB]")
    for axis in axes:
...
def _write_report(path, summary):
    lines = [
        ...
        f"- Electrical-size range: {summary['size_parameter_range'][0]:.4g}--"
        f"{summary['size_parameter_range'][-1]:.4g}",
        f"- RMS RCS error: {summary['rms_error_db']:.4g} dB",
        f"- Maximum absolute RCS error: {summary['maximum_absolute_error_db']:.4g} dB",
        "",
...
def run_validation(
    ...
    outputfile = cache_dir / "pec_sphere_rcs"
    ...
    _save_csv(output_dir / "pec_sphere_backscatter_rcs.csv", result)
    _plot(output_dir / "pec_sphere_backscatter_rcs.png", result)
    summary = {
        "configuration": {
            "dl_metres": DL,
        ...
        "wavelength_cells_range": [
            float(np.min(result["wavelength_cells"])),
            float(np.max(result["wavelength_cells"])),
        ],
...
def main():
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path(__file__).with_name("pec_sphere_rcs_results"),
    )
    parser.add_argument(
        "--cache-dir",
        type=Path,
        default=Path(__file__).with_name("pec_sphere_rcs_results") / "_cache",
    )
    ...
    if not summary["acceptance"]["passed"]:
        raise SystemExit("PEC-sphere RCS analytical validation failed")
