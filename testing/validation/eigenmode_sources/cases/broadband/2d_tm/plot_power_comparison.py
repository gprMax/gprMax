...
def _read_area_plane(
        ...
        if np.isclose(position[0], x_position):
            records.append(
                (
                    float(position[1]),
                    float(position[2]),
                    *(np.asarray(receiver[field], dtype=np.float64) for field in field_names),
                )
            )
    if not records:
        raise ValueError(f"No receiver plane found at x={x_position:g} m")
    ...
    fields = {
        name: np.empty((sample_count, y.size, z.size), dtype=np.float64) for name in field_names
    }
...
def _frequency_domain_line_power(
    ...
    magnetic_spectrum = np.fft.rfft(_align_h_to_e_times(magnetic), n=nfft, axis=0)
    poynting_x = 0.5 * cross_sign * electric_spectrum * np.conj(magnetic_spectrum)
...
def _time_aligned_area_power(
    ...
    poynting_x = ey * _align_h_to_e_times(hz) - ez * _align_h_to_e_times(hy)
...
def _frequency_domain_area_power(
    ...
    hy_spectrum = np.fft.rfft(_align_h_to_e_times(hy), n=nfft, axis=0)
    hz_spectrum = np.fft.rfft(_align_h_to_e_times(hz), n=nfft, axis=0)
    poynting_x = 0.5 * (ey_spectrum * np.conj(hz_spectrum) - ez_spectrum * np.conj(hy_spectrum))
    frequencies = np.fft.rfftfreq(nfft, d=dt)
...
def _normalised_spectrum_db(
    ...
    ratio[source_support] = (
        np.maximum(measured_power[source_support], 0.0) / source_power[source_support]
    )
...
def load_2d_case(
    ...
    upstream_power = _time_aligned_line_power(*upstream, cross_sign=cross_sign)
    downstream_power = _time_aligned_line_power(*downstream, cross_sign=cross_sign)
    ...
    downstream_frequencies, downstream_spectrum = _frequency_domain_line_power(
        *downstream,
        cross_sign=cross_sign,
        dt=dt,
        nfft=nfft,
...
def load_3d_case(
    ...
    nfft = 1 << int(np.ceil(np.log2(max(2, 2 * upstream_power.size))))
    frequencies, upstream_spectrum = _frequency_domain_area_power(*upstream, dt=dt, nfft=nfft)
    downstream_frequencies, downstream_spectrum = _frequency_domain_area_power(
...
def plot_comparison(
    ...
    for color, (label, result) in zip(colors, results.items()):
        ...
        supported_ranges.append((frequencies_ghz[support][0], frequencies_ghz[support][-1]))
        axes[0].plot(
    ...
    raw_spectrum_output = root / "unnormalized_forward_backward_power_spectrum_db.png"
