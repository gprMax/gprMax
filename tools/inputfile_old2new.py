...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Converts old style input file to new style input file.",
        usage="cd gprMax; python -m tools.inputfile_new2old inputfile",
    )
    ...
    print(
        "Attempting to convert inputfile '{}' to use new syntax...\n".format(inputfile)
    )
    ...
    while lindex < len(inputlines):
        if inputlines[lindex].startswith("#") and not inputlines[lindex].startswith(
            ...
            if cmdname == "#dx_dy":
                ...
                replacement = "#dx_dy_dz: {:g} {:g} {:g}".format(
                    float(params[0]), float(params[1]), float(params[1])
                )
                print(
                    "Command '{}', replaced with '{}'".format(
        ...
        if inputlines[lindex].startswith("#") and not inputlines[lindex].startswith(
            ...
            if cmdname == "#domain":
                if model2D:
                    ...
                    replacement = "#domain: {:g} {:g} {:g}".format(
                        float(params[0]), float(params[1]), dx_dy_dz[2]
                    )
                    print(
                        "Command '{}', replaced with '{}'".format(
                            inputlines[lindex], replacement
                ...
                inputlines.insert(lindex, replacement)
                if float(params[1]) > 0:
                    replacement = "#add_dispersion_debye: 1 {} {} {}".format(
                        float(params[0]) - float(params[1]), params[2], params[6]
                    )
                ...
                print(
                    "Command '{}', is no longer supported. You can create the bowtie shape using two triangle commands.".format(
                        inputlines[lindex],
                    )
                )
                inputlines.pop(lindex)
                ...
                print(
                    "Command '{}' has been removed as it is no longer supported. You can create a cylindrical segment by using a #box to cut through a #cylinder.".format(
                        inputlines[lindex]
                    )
                )
                inputlines.pop(lindex)
                ...
                print(
                    "Command '{}' has been removed. A circular segment can be created by using the #cylinder command and cutting it with a #box. Alternatively the #cylindrical_sector command maybe useful.".format(
                        inputlines[lindex]
                    )
                )
                inputlines.pop(lindex)
                ...
                print(
                    "Command '{}' has is no longer supported. Please include your materials using the #material command directly in the input file.".format(
                        inputlines[lindex]
                    )
                )
                inputlines.pop(lindex)
                ...
                "#abc_order",
                "#abc_type",
                "abc_optimisation_angles",
                "#abc_mixing_parameters",
                "#abc_stability_factors",
            ]:
                print(
                    "Command '{}' has been removed as Higdon Absorbing Boundary Conditions (ABC) are no longer supported. The default ABC is the (better performing) Perfectly Matched Layer (PML).".format(
                        inputlines[lindex]
                    )
                )
                inputlines.pop(lindex)
                ...
                if int(params[0]) > 1:
                    extra = " To run a model multiple times use the command line option -n, e.g. gprMax {} -n {}".format(
                        inputfile, int(params[0])
                    )
                else:
                    extra = ""
                print(
                    "Command '{}' has been removed as it is no longer required.{}".format(
                        inputlines[lindex], extra
                    )
                )
                inputlines.pop(lindex)
                ...
                replacement = "#geometry_view: 0 0 0 {} {} {} {} {} {} {} n".format(
                    domain[0],
                    domain[1],
                    domain[2],
                    dx_dy_dz[0],
                ...
                print(
                    "Command '{}', replaced with '{}'. This is a geometry view of the entire domain, sampled at the spatial resolution of the model, using the per Yee cell option (n). You may want to consider taking a smaller geometry view or using a coarser sampling. You may also want to use the per Yee cell edge option (f) to view finer details.".format(
                        inputlines[lindex], replacement
                    )
                )
                inputlines.pop(lindex)
                ...
                raise CmdInputError(
                    "Command '{}' has not yet implemented in the new version of gprMax. For now please continue to use the old version.".format(
                        inputlines[lindex]
                    )
                )
    ...
    for source in linesources:
        ...
        if params[3] is badwaveforms:
            raise CmdInputError(
                "Waveform types {} are not compatible between new and old versions of gprMax.".format(
                    "".join(badwaveforms)
                )
            )
        elif params[3] == "ricker":
            params[3] = "gaussiandotnorm"
        waveform = "#waveform: {} {} {} {}".format(
        ...
        if float(hertziantx[4]) > 0 or float(hertziantx[5]) != timewindow:
            hertzian = "#hertzian_dipole: z {} {} {} {} {} {}".format(
                hertziantx[1],
                hertziantx[2],
                0,
                hertziantx[3],
            ...
            hertzian = "#hertzian_dipole: z {} {} {} {}".format(
                hertziantx[1], hertziantx[2], 0, hertziantx[3]
            )

        print(
            "Commands '{}' and '{}', replaced with '{}' and '{}'".format(
                source, tx, waveform, hertzian
            )
        )
    ...
    for source in hertziandipoles:
        ...
        if float(hertziantx[6]) > 0 or float(hertziantx[7]) != timewindow:
            hertzian = "#hertzian_dipole: {} {} {} {} {} {} {}".format(
                hertziantx[1],
                hertziantx[2],
                hertziantx[3],
                hertziantx[4],
            ...
            hertzian = "#hertzian_dipole: {} {} {} {} {}".format(
                hertziantx[1], hertziantx[2], hertziantx[3], hertziantx[4], hertziantx[5]
            )
    ...
    for source in voltagesources:
        ...
        if params[3] is badwaveforms:
            ...
            params[3] = "gaussiandotnorm"
        waveform = "#waveform: {} {} {} {}".format(
            params[3], params[1], params[2], params[5]
        )
        ...
        if float(voltagesourcetx[6]) > 0 or float(voltagesourcetx[7]) != timewindow:
            voltagesource = "#voltage_source: {} {} {} {} {} {} {} {}".format(
                voltagesourcetx[1],
                voltagesourcetx[2],
                voltagesourcetx[3],
                voltagesourcetx[4],
        ...
        print(
            "Commands '{}' and '{}', replaced with '{}' and '{}'".format(
                source, tx, waveform, voltagesource
            )
        )
        inputlines.remove(source)
        inputlines.remove(tx)
    ...
    for source in transmissionlines:
        ...
        if params[3] is badwaveforms:
            ...
            params[3] = "gaussiandotnorm"
        waveform = "#waveform: {} {} {} {}".format(
            params[3], params[1], params[2], params[6]
        )
        tx = next(tx for tx in txs if tx.split()[5] == params[6])
        transmissionlinetx = tx.split()
        if (
            float(transmissionlinetx[6]) != 0
            or float(transmissionlinetx[7]) < timewindow
        ):
            transmissionline = "#transmission_line: {} {} {} {} {} {} {} {}".format(
                transmissionlinetx[1],
                transmissionlinetx[2],
                transmissionlinetx[3],
                transmissionlinetx[4],
                params[5],
                transmissionlinetx[5],
                transmissionlinetx[6],
                transmissionlinetx[7],
            )
        else:
            transmissionline = "#transmission_line: {} {} {} {} {} {}".format(
                transmissionlinetx[1],
        ...
        print(
            "Commands '{}' and '{}', replaced with '{}' and '{}'".format(
                source, tx, waveform, transmissionline
            )
        )
        inputlines.remove(source)
        inputlines.remove(tx)
