...
def mpl_plot(filename, outputdata, dt, rxnumber, rxcomponent):
    """Creates a plot (with matplotlib) of the Shot Gather.

    Args:
        filename (string): Filename (including path) of output file.
        outputdata (array): Array of Shot Gather.
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Plots a Shot-gather image.',
                                     usage='cd gprMax; python -m tools.plot_ShotGather outputfile output')
    ...
    for rx in range(1, nrx + 1):
        ...
        img.append(outputdata)
    img = np.array(img).transpose()

    plthandle = mpl_plot(args.outputfile, img, dt, rx, args.rx_component)
    plthandle.show()
