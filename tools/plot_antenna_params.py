...
def calculate_antenna_params(
    ...
    if tlrxnumber:
        ...
        if rxcomponent not in availableoutputs:
            raise CmdInputError(
                "{} output requested, but the available output for receiver {} is {}".format(
                    rxcomponent, rxnumber, ", ".join(availableoutputs)
                )
            )
...
def mpl_plot(
    ...
    print(
        "s11 minimum: {:g} dB at {:g} Hz".format(
            np.amin(s11[pltrange]), freqs[s11minfreq + pltrangemin]
        )
    )
    print("At {:g} Hz...".format(freqs[s11minfreq + pltrangemin]))
    print(
        "Input impedance: {:.1f}{:+.1f}j Ohms".format(
            np.abs(zin[s11minfreq + pltrangemin]), zin[s11minfreq + pltrangemin].imag
        )
    )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Plots antenna parameters (s11, s21 parameters and input impedance) from an output file containing a transmission line source.",
        usage="cd gprMax; python -m tools.plot_antenna_params outputfile",
    )
