...
EXPECTED_C_COMPLEX = {
    ...
    ("single", "opencl"): "cfloat",
    ("single", "metal"): "metal::complex<float>",
    ("double", "cpu"): None,
    ("double", "cuda"): "pycuda::complex<double>",
    ("double", "opencl"): "cdouble",
    ("double", "metal"): "metal::complex<double>",
}
...
class TestCComplexPerSolver:
    ...
    def test_opencl_uses_the_short_form(self, make_sim_config):
        assert make_sim_config(opencl=[0]).dtypes["C_complex"] == "cfloat"

    def test_metal_uses_the_namespaced_form(self, make_sim_config):
        assert make_sim_config(metal=[0]).dtypes["C_complex"] == "metal::complex<float>"
...
class TestUnknownPrecision:
    ...
    **never assigned at all**, and because the method is called at the very
    ...
    def test_calling_set_precision_again_with_a_bad_value_leaves_stale_dtypes(
        ...
        Demonstrates the failure shape without asserting a wrong *answer*:
        after an unrecognised precision the table still holds the previous
        run's types, so nothing signals that the request was ignored.
        """
        sim_config = make_sim_config()
        before = dict(sim_config.dtypes)

        sim_config.general["precision"] = "quadruple"
        sim_config._set_precision()

        assert sim_config.dtypes == before
...
class TestConsistencyWithTheKernelDispatch:
    ...
    def test_array_dtype_and_kernel_name_agree(
        ...
        updates = CPUUpdates(SimpleNamespace())
