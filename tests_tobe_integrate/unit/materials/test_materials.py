...
class TestDispersiveDrudeBug:
    ...
    def test_two_poles_match_current_buggy_formula(self, make_dispersive):
        ...
        buggy = 1.0 + cond - 2 * pole0 - pole1
        assert m.calculate_er(f) == pytest.approx(buggy)
...
class TestDispersiveDrudeSelfMutationBug:
    ...
    runs every time ``calculate_update_coeffsE`` is called, so the
    method is not idempotent.

    When fixed, this test should assert the values are equal across calls.
    """
...
class TestCreateBuiltIns:
    def test_appends_pec_and_free_space(self, fake_grid):
        G = fake_grid()
        create_built_in_materials(G)
        assert [m.ID for m in G.materials] == ["pec", "free_space"]
...
class TestListMaterialMissingMaterialBug:
    ...
    Source: ``materials.py:544-548`` — the code calls
    ...
    When fixed, this test should assert ``pytest.raises(ValueError)``.
    """
