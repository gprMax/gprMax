...
class TestModulePathResolution:
    ...
    def test_the_formulation_is_read_at_call_time(self, ready_pml, spy_import):
        """Expects a formulation changed after construction to take effect on
        the next update — the module path is rebuilt on every call rather than
        cached."""
        pml = ready_pml()
        pml.G.pmls["formulation"] = "MRIPML"
        pml.update_electric()
        assert spy_import.modules[0].endswith("MRIPML")
