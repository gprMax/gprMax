...
materials, so every ID read from the file is shifted by
``num_existing_materials``. Without that, an imported object would silently
reference whatever material happened to occupy those indices in the host model.
...
class TestConstruction:
    def test_opens_the_file(self, geometry_file, target_grid):
        ...
        with ReadGeometryObject(geometry_file(), target_grid(), np.zeros(3, np.int32), 0) as r:
    ...
    def test_derives_the_extent_from_the_data_shape(self, geometry_file, target_grid):
        ...
        with ReadGeometryObject(geometry_file(shape=(3, 4, 5)), target_grid(), start, 0) as r:
    ...
    def test_stores_the_material_offset(self, geometry_file, target_grid):
        """Expects the existing-material count kept for use by every read."""
        with ReadGeometryObject(geometry_file(), target_grid(), np.zeros(3, np.int32), 7) as r:
            assert r.num_existing_materials == 7

    def test_is_a_context_manager(self, geometry_file, target_grid):
        """Expects ``__enter__`` to return the reader itself."""
        reader = ReadGeometryObject(geometry_file(), target_grid(), np.zeros(3, np.int32), 0)
        with reader as r:
            assert r is reader
...
class TestValidation:
    def test_matching_discretisation_is_accepted(self, geometry_file, target_grid):
        ...
        with ReadGeometryObject(
            geometry_file(dl=(DL, DL, DL)), target_grid(dl=DL), np.zeros(3, np.int32), 0
        ) as r:
    ...
    def test_a_single_mismatched_axis_is_rejected(self, geometry_file, target_grid):
        ...
        with ReadGeometryObject(
            geometry_file(dl=(DL, DL, 2 * DL)), target_grid(dl=DL), np.zeros(3, np.int32), 0
        ) as r:
            assert not r.has_valid_discritisation()
    ...
    def test_detects_an_id_array(self, geometry_file, target_grid):
        ...
        with ReadGeometryObject(geometry_file(include_id=True), target_grid(), np.zeros(3, np.int32), 0) as r:
    ...
    def test_detects_a_missing_id_array(self, geometry_file, target_grid):
        ...
        with ReadGeometryObject(geometry_file(include_id=False), target_grid(), np.zeros(3, np.int32), 0) as r:
    ...
    def test_detects_rigid_arrays(self, geometry_file, target_grid):
        ...
        with ReadGeometryObject(geometry_file(include_rigid=True), target_grid(), np.zeros(3, np.int32), 0) as r:
    ...
    def test_detects_missing_rigid_arrays(self, geometry_file, target_grid):
        ...
        with ReadGeometryObject(geometry_file(include_rigid=False), target_grid(), np.zeros(3, np.int32), 0) as r:
...
class TestReadData:
    def test_writes_the_solid_array_into_the_grid(self, geometry_file, target_grid):
        ...
        requested position."""
        g = target_grid()
        with ReadGeometryObject(geometry_file(shape=(3, 3, 3)), g, np.zeros(3, np.int32), 0) as r:
            r.read_data()
    ...
    def test_places_the_object_at_the_requested_start(self, geometry_file, target_grid):
        ...
        with ReadGeometryObject(geometry_file(shape=(3, 3, 3)), g, start, 0) as r:
            r.read_data()
        assert g.solid[4, 4, 4] == 26
        assert g.solid[0, 0, 0] == 0

    def test_shifts_material_ids_by_the_existing_count(self, geometry_file, target_grid):
        """Expects every imported ID increased by ``num_existing_materials``,
        so it names the material the importing model just added rather than one
        it already had."""
        g = target_grid()
        with ReadGeometryObject(geometry_file(shape=(3, 3, 3)), g, np.zeros(3, np.int32), 10) as r:
            r.read_data()
    ...
    def test_get_data_does_not_apply_the_material_offset(self, geometry_file, target_grid):
    ...
    def test_data_is_converted_to_int16(self, geometry_file, target_grid):
        ...
        g = target_grid()
        with ReadGeometryObject(
            geometry_file(shape=(3, 3, 3), data_dtype=np.uint16), g, np.zeros(3, np.int32), 0
        ) as r:
            data = r.get_data()
    ...
    def test_an_int16_file_is_left_alone(self, geometry_file, target_grid):
        ...
        g = target_grid()
        with ReadGeometryObject(
            geometry_file(shape=(3, 3, 3), data_dtype=np.int16), g, np.zeros(3, np.int32), 0
        ) as r:
...
class TestReadRigidAndId:
    ...
    def test_id_is_material_shifted(self, geometry_file, target_grid):
        """Expects the offset applied here, unlike the rigid arrays — ``ID``
...
class TestRoundTrip:
    def test_an_exported_object_reads_back_unchanged(
        ...
        target = target_grid()
        with ReadGeometryObject(obj.filename_hdf5, target, np.zeros(3, np.int32), 0) as r:
            assert r.has_valid_discritisation()
            r.read_data()
            r.read_ID()
    ...
    def test_the_round_trip_respects_a_material_offset(
        ...
        target = target_grid()
        with ReadGeometryObject(obj.filename_hdf5, target, np.zeros(3, np.int32), 5) as r:
            r.read_data()
...
class TestMpiPaths:
    ...
    def test_an_overlapping_rank_gets_an_mpi_grid_view(self, geometry_file, as_mpi_grid):
        ...
        with ReadGeometryObject(
            geometry_file(shape=(3, 3, 3)), as_mpi_grid, np.zeros(3, np.int32), 0
        ) as r:
