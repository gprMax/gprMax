...
class TestBuildBox:
    def test_averaging_writes_solid_and_clears_rigid(self, grid_arrays):
        ...
        build_box(1, 4, 1, 3, 1, 2, NUM_ID, NUM_IDX, NUM_IDY, NUM_IDZ, True,
    ...
    def test_hard_box_stamps_interior_and_trailing_faces(self, grid_arrays):
        ...
        build_box(xs, xf, ys, yf, zs, zf, NUM_ID, NUM_IDX, NUM_IDY, NUM_IDZ,
                  False, g.solid, g.rigidE, g.rigidH, g.ID)
        ...
        expected = {}
        for i, j, k in box:
            expected[(0, i, j, k)] = NUM_IDX
            expected[(1, i, j, k)] = NUM_IDY
            expected[(2, i, j, k)] = NUM_IDZ
            expected[(3, i, j, k)] = NUM_IDX
            expected[(4, i, j, k)] = NUM_IDY
            expected[(5, i, j, k)] = NUM_IDZ
        # Trailing writes closing the far (+x / +y / +z) faces of the box.
        for i in range(xs, xf):
            expected[(0, i, yf, zf)] = NUM_IDX
        for j in range(ys, yf):
            expected[(1, xf, j, zf)] = NUM_IDY
        for k in range(zs, zf):
            expected[(2, xf, yf, k)] = NUM_IDZ
        for j in range(ys, yf):
            for k in range(zs, zf):
                expected[(3, xf, j, k)] = NUM_IDX
        for i in range(xs, xf):
            for k in range(zs, zf):
                expected[(4, i, yf, k)] = NUM_IDY
        for i in range(xs, xf):
            for j in range(ys, yf):
                expected[(5, i, j, zf)] = NUM_IDZ
    ...
    def test_empty_range_is_a_noop(self, grid_arrays):
        g = grid_arrays(4, 4, 4)
        build_box(2, 2, 0, 4, 0, 4, NUM_ID, NUM_IDX, NUM_IDY, NUM_IDZ, True,
                  g.solid, g.rigidE, g.rigidH, g.ID)
    ...
    def test_full_domain_box(self, grid_arrays):
        g = grid_arrays(4, 4, 4)
        build_box(0, 4, 0, 4, 0, 4, NUM_ID, NUM_IDX, NUM_IDY, NUM_IDZ, True,
                  g.solid, g.rigidE, g.rigidH, g.ID)
...
class TestBuildSphere:
    def test_radius_of_one_cell_marks_the_eight_corner_cells(self, grid_arrays):
        ...
        build_sphere(4, 4, 4, DL, DL, DL, DL, NUM_ID, NUM_IDX, NUM_IDY,
                     NUM_IDZ, True, g.solid, g.rigidE, g.rigidH, g.ID)
    ...
    def test_cells_match_the_inside_check(self, grid_arrays):
        ...
        build_sphere(4, 4, 4, r_cells * DL, DL, DL, DL, NUM_ID, NUM_IDX,
                     NUM_IDY, NUM_IDZ, True, g.solid, g.rigidE, g.rigidH, g.ID)
    ...
    def test_hard_sphere_sets_rigid_at_written_cells(self, grid_arrays):
        g = grid_arrays()
        build_sphere(4, 4, 4, DL, DL, DL, DL, NUM_ID, NUM_IDX, NUM_IDY,
                     NUM_IDZ, False, g.solid, g.rigidE, g.rigidH, g.ID)
...
class TestBuildEllipsoid:
    def test_cells_match_the_ellipsoid_equation(self, grid_arrays):
        ...
        build_ellipsoid(4, 4, 4, xr * DL, yr * DL, zr * DL, DL, DL, DL,
                        NUM_ID, NUM_IDX, NUM_IDY, NUM_IDZ, True,
                        g.solid, g.rigidE, g.rigidH, g.ID)
...
class TestBuildCone:
    def test_z_aligned_cone_shrinks_layer_by_layer(self, grid_arrays):
        ...
        # ...and the narrowest is the 2x2 core.
        assert nonzero_set(g.solid[:, :, 5]) == {
            (i, j) for i in (4, 5) for j in (4, 5)
        }
    ...
    def test_equal_radii_reduce_to_a_cylinder(self, grid_arrays):
        ...
        build_cone(*p1, *p2, r, r, DL, DL, DL, NUM_ID, NUM_IDX, NUM_IDY,
                   NUM_IDZ, True, g_cone.solid, g_cone.rigidE, g_cone.rigidH,
                   g_cone.ID)
        build_cylinder(*p1, *p2, r, DL, DL, DL, NUM_ID, NUM_IDX, NUM_IDY,
                       NUM_IDZ, True, g_cyl.solid, g_cyl.rigidE, g_cyl.rigidH,
                       g_cyl.ID)
