...
class TestPMLFormulation:
    def test_formulation_string_stored(self, singlecmds_template):
        singlecmds_template["#pml_formulation"] = "HORIPML"
        objs = process_singlecmds(singlecmds_template)
        assert isinstance(objs[0], PMLFormulation)
        assert objs[0].formulation == "HORIPML"

    def test_multi_token_rejected(self, singlecmds_template):
        singlecmds_template["#pml_formulation"] = "HORIPML extra"
        with pytest.raises(ValueError):
            process_singlecmds(singlecmds_template)
...
class TestPMLCellsSixArgBranchBug:
    """Bug tripwire: ``hash_cmds_singleuse.py:147-153``.

    ``PMLThickness.__init__`` (``cmds_singleuse.py:355``) takes a single
    positional ``thickness`` parameter. The 6-token ``#pml_cells`` branch
    constructs it with per-face kwargs ``x0=, y0=, z0=, xmax=, ymax=,
    zmax=`` — unknown kwargs, so a ``TypeError`` is raised before any
    object reaches the scene list.

    This test pins the current crash. When the dispatcher is fixed to use
    ``thickness=(x0, y0, z0, xmax, ymax, zmax)`` (or ``PMLThickness`` is
    expanded to accept per-face kwargs), the tripwire fails and the
    expectation should flip to a passing assertion on a six-tuple.
    """

    def test_six_token_form_currently_raises_type_error(self, singlecmds_template):
